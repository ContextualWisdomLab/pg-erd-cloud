...
def _run_validator(script_path: str) -> dict[str, Any]:
...
def _real_evidence_gate(rule: dict[str, Any]) -> dict[str, Any]:
    ...
    status = "ready" if real_files else "no_go"
    return {
        "id": rule["id"],
        "label": rule["label"],
        "required_for": rule["required_for"],
        "status": status,
        "real_evidence_files": [_relative(path) for path in real_files],
        "example_files": [_relative(path) for path in example_files],
        "no_go_reason": None if real_files else rule["no_go_reason"],
    }
...
def generate_report() -> dict[str, Any]:
    ...
    evidence_gates = [_real_evidence_gate(rule) for rule in REAL_EVIDENCE_RULES]
