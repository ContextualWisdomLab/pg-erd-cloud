"""Apply deterministic review fixes to PR 696's frontend regression tests."""
...
def replace_once(source: str, old: str, new: str, label: str) -> str:
    """Replace one exact review target and fail if the branch has drifted."""
...
def main() -> None:
    """Patch search, invalid-form, and runtime DSN fixture coverage."""
    ...
    source = replace_once(
        ...
        "    const searchInput = screen.getByLabelText('다이어그램 검색')\n"
        "    await waitFor(() => expect(screen.queryByText('아직 다이어그램 스냅샷이 없습니다. 편집기에서 데이터베이스를 역공학해 시작하세요.')).not.toBeInTheDocument())\n"
        "    const diagramApiCalls = api.listSnapshots.mock.calls.length\n"
        "    const diagramHistoryLength = window.history.length\n"
        "    await user.clear(searchInput)\n"
        "    await user.type(searchInput, 'no-match{Enter}')\n"
        "    expect(screen.getByText('검색 결과가 없습니다.')).toBeInTheDocument()\n"
        "    expect(api.listSnapshots).toHaveBeenCalledTimes(diagramApiCalls)\n"
    ...
    source = replace_once(
        ...
        "    await user.type(dsn, '{Enter}')\n"
        "    expect(api.createConnection).not.toHaveBeenCalled()\n\n"
        "    const malformedDsn = buildTestDsn('postgresql', '[bad-host')\n"
        ...
        "editor invalid form and DSN fixture coverage",
    )
    ...
    source = replace_once(
        ...
        "    const canvasSearchInput = screen.getByLabelText('테이블 또는 컬럼 검색')\n"
        "    const canvasApiCalls = api.getSnapshot.mock.calls.length\n"
        ...
        "    expect(screen.getByText('1개 테이블 일치', { exact: false })).toBeInTheDocument()\n"
        "    expect(api.getSnapshot).toHaveBeenCalledTimes(canvasApiCalls)\n"
    ...
    for forbidden in forbidden_literals:
        if forbidden in source:
            raise RuntimeError(f"complete DSN literal remains: {forbidden}")
