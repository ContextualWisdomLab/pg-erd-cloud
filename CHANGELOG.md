...
## Unreleased
- [BE/FE] 🔒 **현재 기준 공급망·테스트 실행 정합화**: frontend nanoid를 3.3.18로 고정하고 backend 테스트 import 경로를 프로젝트 설정에 선언했으며, 사용하지 않는 관계 추론 지역 변수를 제거했습니다.
- [FE] 🧪 **비동기 UI 회귀 검증 안정화**: 검색 결과 렌더링을 상태 반영 후 검증하고, 자원 경합 환경에서 전체 frontend 테스트가 조기 timeout으로 오판되지 않도록 기본 테스트 제한을 15초로 조정했습니다.
- [FE] ♿ **네이티브 폼 키보드 제출**: 편집기·프로젝트 목록의 프로젝트 생성과 연결 생성은 한 개의 guarded `submit` 경로를 사용해 Enter와 클릭이 정확히 한 번 동작합니다. 다이어그램·캔버스 검색은 `role="search"` 폼에서 Enter 내비게이션만 막고 실시간 필터를 유지합니다.
