...
## Unreleased
- [FE/BE] 🧩 **Prisma 식별자 충돌 없는 할당**: 예약어·구두점·유니코드 테이블/컬럼 이름을 Prisma 문법에 맞게 고유 할당하고, 원본 이름은 `@map`/`@@map`으로 보존합니다. 할당에 실패하면 고정 안내와 진단 매니페스트만 제공합니다. 다음 행동: 내보낸 `.prisma`를 프로젝트에 넣은 뒤 `prisma validate`를 실행하세요.
