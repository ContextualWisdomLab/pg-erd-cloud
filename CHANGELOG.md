...
- [FE] ♿ **프로젝트 생성 진행 상태 접근성 강화**: 생성 중인 버튼에 `aria-busy`를 노출해 보조기기가 작업 진행 상태를 안내할 수 있도록 했습니다.
- [FE] 🔒 **nanoid 공급망 패치 고정**: 간접 의존성을 3.3.18 이상으로 고정해 zero-size custom generator 무한 루프 취약점(GHSA-2v37-7h3g-55p8)을 제거합니다.
- [FE] 🧪 **프론트엔드 테스트 안정화**: 통합 UI 테스트를 순차 실행하고 비동기 상호작용의 제한 시간을 명시해 병렬 실행으로 인한 간헐적 타임아웃을 줄였습니다.
