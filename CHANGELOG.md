...
## Unreleased
- [BE] 🔁 **APP_SECRET dual-read / single-write 회전 코어 (#946 2차 증분)**: `app/secret_provider/rotation.py`를 추가했습니다(DB·Settings·I/O 없음). `dual_read_decrypt(key_set, ciphertext, nonce)`는 후보 `APP_SECRET`을 순서대로 시도해(HKDF 키 → 레거시 SHA-256 키, `app.security`와 동일) 하나로 복호합니다. `plan_key_rotation(active_key, previous_keys, records, *, nonce_source=os.urandom)`는 각 저장 ciphertext를 어떤 키로든 복호해 active 키로 재암호화하고, 이미 active 키인 행은 그대로 두며, 어떤 키로도 복호 불가/형식 오류 행은 `needs_key_recovery`로 노출합니다(누락·추측 재암호화 없음). 결과에 평문·키 없음. `app.security.encrypt_text` 실제 blob을 이 모듈로 회전 후 `app.security.decrypt_text`로 복호하는 왕복 테스트로 HKDF 파생 일치를 강제(10종). `docs/doctoring/credential-provider-contract.md` 회전 설계 절 갱신.
