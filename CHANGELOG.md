...
## Unreleased
- [FE] 🔒 **프런트엔드 nanoid 보안 경계 갱신**: `nanoid`를 3.3.18 이상으로 고정해 현재 npm high advisory를 제거하고 lockfile 설치 경로를 검증했습니다.
- [BE] 🧭 **PostgreSQL 18 예약어 정합화**: 공식 PostgreSQL 키워드 표에 맞춰 `SYSTEM_USER`를 대소문자와 무관하게 high-severity 예약어로 분류하고, 정확한 스키마·테이블 대상을 회귀 테스트로 고정합니다.
