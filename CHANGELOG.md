...
## Unreleased
- [FE] ♿ **공유 접근 안내 의미론 교정**: 실제 권한 관리 route·handler·API가 없는 `ExportModal`에서 동작하지 않는 `접근 관리` 버튼을 제거하고, 동일 안내를 읽기 순서의 명명된 `note`로 제공합니다. 가짜 CTA와 불필요한 keyboard focus stop을 만들지 않는 회귀 테스트를 추가했습니다.
