...
## Unreleased
- [BE] 🧬 **스냅샷 lineage PROV-JSON 투영 (#948 2차 증분)**: `app/lineage/prov_projection.py`의 `to_prov_document(lineage_graph, *, base_uri="urn:pg-erd-cloud:")`를 추가했습니다. `build_lineage_graph` 결과를 W3C PROV-JSON 문서로 순수 투영합니다 — 스냅샷 id(노드 + dangling parent 참조 포함)마다 `pg:` prefix의 `prov:Entity`, 타입드 derivation edge마다 `wasDerivedFrom`(`_:wdf<N>`) 한 건(`prov:generatedEntity`=child, `prov:usedEntity`=parent, `pg:derivationKind`=edge 종류)으로 "by kind" 정보를 보존합니다. PROV-JSON은 평문 JSON이라 신규 의존성 없음, `(kind, child, parent)` 순으로 번호를 매겨 결정론적, `json.dumps` 직렬화 가능. 내부 그래프가 authoritative하고 PROV 문서는 읽기 전용 뷰입니다. 테스트 9종.
- [BE] 🧬 **스냅샷 lineage·promotion 모델 (1차 증분)**: `app/lineage/` 패키지에 순수 모델·알고리즘을 추가했습니다(DB·I/O 없음). bitemporal 시간모델(`captured_at`/`available_at`/`valid_from`/`valid_to`/`recorded_at`/`superseded_at`/`knowledge_cutoff`을 별개 사실로 분리), 타입드 derivation edge(`captured_from`/`imported_from`/`normalized_from`/`compared_with`/`exported_from`/`planned_from` — 단일 generic edge 금지)로 DAG를 구성하는 `build_lineage_graph`(cycle 거부·orphan/dangling 보고·위상정렬), append-only optimistic-concurrency `apply_promotion`(이전 유효구간을 재작성 없이 종료, stale version 충돌 거부), 삭제 없이 disposition만 반환하는 `decide_retention`(promoted baseline·legal hold 보호). 테스트 9종. 영속화·HTTP·PROV 프로젝션은 후속 증분(#948). `docs/doctoring/snapshot-lineage-and-promotion.md`.
