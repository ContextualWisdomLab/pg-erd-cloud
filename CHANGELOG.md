...
## Unreleased
- [BE] 🧭 **Job queue 상태 이름 의미화**: 애플리케이션 소유 `job_queue.status`를 `job_queue.job_status`로 바꾸고 SQLAlchemy/worker의 authoritative vocabulary와 `FOR UPDATE SKIP LOCKED` 인덱스를 같은 bounded-context 이름으로 정합화했습니다. Alembic은 데이터/인덱스 재빌드 없이 PostgreSQL metadata와 기존 인덱스 이름만 가역적으로 변경합니다.
