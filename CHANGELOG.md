...
## Unreleased
- [FE] ♿ **사이드바 생성 폼 Enter 제출 지원**: 편집기에서 새 프로젝트와 새 데이터베이스 연결을 입력한 뒤 포인터로 버튼을 누르지 않아도 Enter로 같은 생성 경계를 실행할 수 있게 했습니다. 빈 입력과 생성 중 중복 제출 방지 계약은 기존 핸들러에 유지하고, 실제 입력에서 Enter를 보내는 회귀 테스트로 고정했습니다.
