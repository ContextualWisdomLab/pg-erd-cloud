...
## Unreleased
- [FE] **관계 기본 라벨과 연결 컬럼 검증**: ERD에서 컬럼을 연결할 때 계산을 순수 helper로 분리하고, 기본 라벨·endpoint 컬럼 메타데이터를 채우며, 잘못된 Unicode handle 토큰을 거부합니다.
- [Test] **Coverage 실행 안정화**: 계측으로 느려지는 UI orchestration 테스트에 충분한 제한 시간을 부여해 실제 비동기 경로가 timeout flake로 오인되지 않도록 했습니다.
