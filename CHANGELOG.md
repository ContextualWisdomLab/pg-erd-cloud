...
## Unreleased
- [FE] ♿ **비활성 export 동작 설명 가능성 개선**: 데이터가 없어 사용할 수 없는 export 및 접근관리 동작을 탭 순서에서 제거하지 않고 `aria-disabled`와 가시적 사유 설명으로 노출합니다. 클릭·Enter·Space 콜백은 명시적으로 차단되며, 사용 가능해지면 정상 실행됩니다.
...
- [FE] 🪄 **관계 자동 추론 기능 추가**: 컬럼 이름(e.g. `user_id`)을 분석하여 연관된 테이블에 자동으로 Foreign Key Edge를 연결하는 버튼을 툴바에 추가했습니다.
...
- [FE] ⬇️ **DBML Export**: ERD 다이어그램을 DBML 형식으로 내보낼 수 있는 기능을 추가했습니다.
