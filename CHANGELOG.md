...
## Unreleased
- [BE] 🔒 **DSN 오류 메시지 비밀값 redaction 강화**: URL user-information과 form-query 디코딩 규칙을 분리하고, raw·decoded·canonical encoding 후보를 마스킹합니다. 짧은 Unicode/구두점 비밀값은 더 큰 단어를 훼손하지 않으며, malformed·scheme-less DSN에서도 fail-closed best-effort redaction을 유지합니다.
