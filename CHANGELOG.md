...
## Unreleased
- [FE] 🔗 **관계 추론 식별자 보존**: 점·따옴표가 포함된 PostgreSQL relation 이름을 별도 식별자로 유지해 자동 관계 추론의 대상 누락을 방지합니다.
- [FE/BE] 🛠️ **프로젝트 전환·스냅샷 폴링 안정화**: 프로젝트를 전환하거나 선택 해제할 때 이전 연결·스냅샷·캔버스·오류 상태를 즉시 비우고, 완료·실패한 폴링의 중복 요청과 늦은 응답을 차단했습니다. 사용하지 않는 관계 추론 lookup을 제거하고 frontend 의존성 보안 override를 유지합니다.
