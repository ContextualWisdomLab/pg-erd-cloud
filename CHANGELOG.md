...
- [FE] ⚡ **검색 노드 참조·텍스트 캐시 안정화 및 순차 스냅샷 폴링**: 같은 정규화 검색어와 immutable 원본 테이블 데이터에는 장식된 `node.data`와 lower-cased searchable text를 재사용하여 드래그 중 반복 컬럼 순회·문자열 할당·하위 렌더링을 줄입니다. 검색 가능한 데이터가 바뀌면 새 identity에서 다시 계산합니다. 스냅샷 폴링은 이전 요청이 끝난 뒤에만 다음 요청을 예약하며, 선택 변경·언마운트 후 도착한 오래된 성공 또는 실패 응답을 무시합니다.
