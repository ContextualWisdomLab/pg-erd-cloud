...
## Unreleased
- [FE] 🧭 **PostgreSQL 식별자 충실도 복구**: 관계 자동 추론이 이미 존재하는 테이블 키를 ASCII 전용 sanitizer로 다시 쓰지 않도록 수정하고, snapshot node에 원본 `relation_name`을 별도로 보존하여 유니코드·공백·점이 포함된 quoted-style 식별자도 정확한 대상 테이블로 연결합니다.
