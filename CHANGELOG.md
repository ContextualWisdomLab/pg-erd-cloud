...
## Unreleased
- [BE] 🔒 **DSN 비밀정보 오류 메시지 redaction 강화**: URL userinfo와 form query의 `+` 의미를 분리하고, 인코딩 변형과 짧은 비밀정보의 Unicode 토큰 경계를 검증하여 누출과 과잉 마스킹을 함께 방지합니다.
...
- [FE] 📋 **테이블 복제 기능 추가**: 편집 모달 내에서 기존 테이블의 구조(컬럼 정보 포함)를 그대로 복사하여 새 테이블 노드로 생성하는 '복제' 버튼을 편집 모달에 추가했습니다.
