...
- [BE] ⚡ **Backend grouping temporary allocation reduction (Proposed)**: `backend/app/ddl/export.py`와 `backend/app/spec/orm_codegen.py`의 mutable-default `setdefault`를 명시적 key 검사로 바꿔, N개 입력·U개 고유 key에서 최대 `N−U`개의 사용되지 않는 임시 collection 생성을 피합니다. 반복 복잡도는 Θ(N)으로 유지됩니다. CPU·GC 개선은 동일 환경의 paired baseline/current median·p95 측정 전까지 주장하지 않습니다.
