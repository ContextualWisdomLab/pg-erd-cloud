...
- [BE] 🧪 **Pydantic alias 정적 타입 계약 복구**: `pydantic.mypy`를 backend mypy 구성에 활성화해 `populate_by_name=True`인 모델의 내부 `diagram_name=` 생성자 계약을 정적 검사에도 반영합니다. HTTP `name` alias와 runtime 검증은 그대로 유지하며 `call-arg`를 suppress하지 않습니다.
