...
## Unreleased
- [FE] 🧭 **관계 기반 ERD 자동 정렬**: 스냅샷 최초 배치와 `ERD 자동 정렬`이 동일한 계층형 엔진을 사용해 참조 대상 테이블을 종속 테이블보다 먼저 배치합니다. 순환·병렬 관계·독립 테이블·누락 엔드포인트·가변 크기를 결정론적으로 처리하고, 불완전한 좌표가 발생하면 기존 위치와 Undo 경계를 그대로 보존합니다.
- [Docs/Legal] 📜 **상용 이용 가능 레이아웃 라이선스 경계**: `@dagrejs/dagre` 3.1.0과 `@dagrejs/graphlib` 4.0.3을 정확히 고정하고 두 MIT 고지문을 보존합니다. CI가 버전·라이선스·전이 런타임 의존성·고지문 드리프트를 오프라인에서 거부합니다.
