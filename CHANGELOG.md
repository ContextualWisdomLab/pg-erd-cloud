...
## Unreleased
- [BE] Forward Engineering durable-run 기반: `migration_run`·`migration_run_event` 영속화, idempotency digest/상태/순서 DB 제약, dry-run/apply 상태 전이 계약, SQL·DSN·credential 필드를 거부하는 bounded evidence canonicalizer를 추가했습니다. 공개 run API·queue worker·실제 dry-run/apply는 아직 Planned입니다.
