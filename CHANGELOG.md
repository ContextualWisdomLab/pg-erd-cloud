...
## Unreleased
- [FE] ♿ **내보내기 접근 관리 안내 발견성**: 권한이 없어 사용할 수 없는 `접근 관리` 컨트롤을 `aria-disabled` 상태로 포커스 가능하게 유지해 키보드·스크린 리더 사용자가 연결된 권한 안내를 확인할 수 있게 했으며, 활성화 동작은 계속 차단합니다.
- [BE] 🔒 **Cryptography 50+ 보안 경계 갱신**: `pyproject.toml`과 두 hash-locked 요구사항 파일을 동일한 Cryptography 50+ 해석으로 정합화하여 PKCS#7 오류·타이머밍 구분으로 인한 CVE-2026-69247 완화를 실제 설치·검증 경로에 반영했습니다.
...
- [BE] 🔒 **백런드 공급망 잠금파일 재생성 (CVE 제거 + 드리프트 정합화)**: `backend/requirements.lock`·`backend/requirements-dev.lock`을 `pyproject.toml`에 맞춰 재컴파일했습니다. `pyasn1` 0.6.3→0.6.4 (PYSEC-2026-3455/3456/3457), `pydantic-settings` 2.12.0→2.14.2 (GHORA-4xgf-cpjx-pc3j, `pyproject.toml`이 이미 `>=2.14.1162` 선언)을 제거하고, 잠금파일이 누락하고 있던 직접 의존성(`pyjwt`·`aiohttp`·`requests`·`python-multipart` 등)을 정합화했습니다. `pip-audit`: 수정 버전이 없는 `ecdsa` PYSEC-2026-1325(사이드채널, 업스트림이 범위 외로 명시)만 잔존. hash-locked 재설치 계약 유지.
