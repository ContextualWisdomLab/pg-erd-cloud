...
## Unreleased
- [FE] 🛠️ **dev 서버 CSP로 인한 무스타일 렌더 수정 (#1108)**: `vite dev`가 주입하는 인라인 HMR 스타일이 `style-src 'self'`에 차단되어 개발 서버에서 앱이 무스타일로 보이던 문제를 `apply: 'serve'` 전용 플러그인으로 해소했습니다. 프로덕션 빌드 산출물의 CSP는 그대로 strict를 유지합니다.
