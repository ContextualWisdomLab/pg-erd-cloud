...
## Unreleased
- [FE/Test] 🧪 **Browser-suite timeout budget**: Vitest UI tests now allow 15 seconds per case so parallel CI resource contention is reported as a real assertion failure instead of a false timeout.
- [BE/CI] 🔒 **Pinned PostgreSQL repair service image**: the Alembic lock repair workflow now uses the immutable `postgres:18.1-alpine` digest recorded by the registry, preventing mutable-tag drift in CI.
