...
## Unreleased
- [FE] 🧭 **점 포함 PostgreSQL relation 식별자 보존**: 스냅샷 그래프가 표시 제목과 원본 relation 이름을 분리해 `Order.Items_id` 같은 외래키 추론을 정확히 유지하며, quoted relation 회귀 테스트를 추가했습니다. nanoid 3.3.18 override로 프런트엔드 공급망 경고도 제거했습니다.
- [FE] 🧭 **관계 추론의 PostgreSQL 식별자 보존**: 자동 관계 추론은 이미 발견된 테이블 이름을 ASCII allowlist로 다시 쓰지 않고 정확한 키로 조회합니다. 따라서 따옴표가 필요한 공백·대소문자 혼합·Unicode 테이블 이름도 손실 없이 연결되며, 해당 동작을 회귀 테스트로 고정했습니다.
