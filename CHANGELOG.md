...
## Unreleased
- [FE] ⚡ **ERD 핸들 ID 생성의 중간 배열 제거**: 컬럼 이름의 Unicode code-point 기반 핸들 문자열은 그대로 유지하면서 반복 생성 경로의 `Array.from` 중간 배열을 제거했습니다. 지원 Node 26 런타임에서 실행 가능한 순서 보존 paired benchmark와 원시 증거 계약을 함께 제공합니다.
