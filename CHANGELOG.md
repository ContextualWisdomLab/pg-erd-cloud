...
## Unreleased
- [BE] 🔒 **OIDC JWK 검증 경계 정합화**: PyJWT가 RSA/EC JWK를 실제 공개 키 객체로 변환한 뒤 고정 알고리즘 allowlist와 top-level `leeway`로 검증하도록 수리했습니다. 유효한 RSA JWK fixture가 변환 경계를 직접 검증하며, `python-jose`·`types-python-jose` 및 전이 의존성을 선언·hash-locked 설치 경로에서 제거했습니다.
