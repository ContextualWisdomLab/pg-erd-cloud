...
## Unreleased
- [FE] ⚡ **ERD handle ID hot path 최적화**: Unicode code-point 기반 식별자와 `c-empty` 호환성을 그대로 유지하면서 `Array.from(...).join()` 중간 배열을 제거했습니다. counterbalanced benchmark는 pair 순서·raw elapsed time·진단용 V8 heap delta를 보존하며 Node 26 exact-head 재실행을 요구합니다.
