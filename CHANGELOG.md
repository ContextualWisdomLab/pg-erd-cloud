...
## Unreleased
- [BE] 🔒 **PostgreSQL DSN 로컬 파일 경계 강화**: 사용자 DSN의 `passfile`, `sslcert`, `sslcrl`, `sslkey`, `sslrootcert`를 DNS·드라이버 처리 전에 고정 오류로 거부하여 서버 파일 탐색·읽기와 경로 반사를 차단합니다. 커스텀 인증서는 향후 서버 소유 secret provider 경계로 제한합니다.
