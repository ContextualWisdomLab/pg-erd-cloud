...
## Unreleased
- [FE] ♿ **모달 작업 버튼의 맥락 보강**: 관계 삭제와 테이블 삭제·복제 버튼의 accessible name에 대상 식별자를 포함해 키보드·스크린 리더 사용자가 다음 작업을 구분할 수 있게 합니다.
- [BE] 🔒 **Cryptography 50+ 보안 경계 갱신**: `pyproject.toml`과 두 hash-locked 요구사항 파일을 동일한 Cryptography 50+ 해석으로 정합화하여 보안 패치 경로를 유지합니다.
- [BE] 🧪 **테스트 실행 경로 고정**: pytest가 backend 패키지를 프로젝트 설정에서 직접 찾도록 하여 개발자가 수동 `PYTHONPATH` 설정 없이 동일한 테스트를 실행합니다.
