...
- [FE] ♿ **비활성 공유·내보내기 작업의 발견 가능성**: 현재 실행할 수 없는 공유·내보내기 작업을 포커스 순서에서 제거하지 않고 `aria-disabled`와 화면에 보이는 선행조건 안내를 함께 제공합니다. Storybook은 선행조건 누락·생성 중·준비됨·오류·좁은 화면 상태를 실제 `ExportModal` 컴포넌트로 고정하며, Figma handoff와 동일한 디자인 토큰/다음 행동 계약을 사용합니다.
