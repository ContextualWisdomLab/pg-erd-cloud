...
## 2026-08-04 - STRIX Pentest DSN Redaction Fixes\n**Vulnerability:** DSN Redaction logic contained critical and high flaws including missing schema-less fallback, double-encoded value bypasses, poor handling of unencoded ampersands, and missing embedded-DSN extraction.\n**Learning:** Robust DSN parsing requires full URL parameter handling (), recursive unquoting, and defensive fallbacks. Blind  functions on URIs create huge vulnerability windows.\n**Prevention:** Rely on standard parsing libraries (), recursively unquote query values, and use dedicated regexes () to sweep full log messages rather than trusting the supplied DSN parameter alone.

## 2026-08-04 - STRIX Pentest DSN Redaction Fixes
**Vulnerability:** DSN Redaction logic contained critical and high flaws including missing schema-less fallback, double-encoded value bypasses, poor handling of unencoded ampersands, and missing embedded-DSN extraction.
**Learning:** Robust DSN parsing requires full URL parameter handling (`parse_qsl`), recursive unquoting, and defensive fallbacks. Blind `split` functions on URIs create huge vulnerability windows.
**Prevention:** Rely on standard parsing libraries (`urllib.parse`), recursively unquote query values, and use dedicated regexes (`_DSN_PATTERN`) to sweep full log messages rather than trusting the supplied DSN parameter alone.
