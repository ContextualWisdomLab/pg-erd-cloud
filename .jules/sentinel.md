...
## 2026-07-09 - Hardcoded Secret Key Derivation (CWE-326)
**Vulnerability:** The `app.security` module derived AES encryption keys from the `APP_SECRET` using a basic `hashlib.sha256(settings.app_secret).digest()`. This lacks salting and proper key expansion, rendering the encryption keys vulnerable to dictionary attacks and static key reuse vulnerabilities if the base secret is leaked.
**Learning:** Raw hash functions like `sha256` are not designed for Key Derivation Functions (KDFs). They lack the cryptographic stretching and salting mechanisms required to generate secure, uniformly distributed keys for symmetric encryption algorithms like AES-GCM.
**Prevention:** Always use a proper, standard KDF like HKDF (HMAC-based Extract-and-Expand Key Derivation Function) or PBKDF2 when deriving cryptographic keys from passwords or shared secrets. Use a unique, randomly generated salt for every encryption operation and securely store or prepend the salt to the ciphertext so the key can be correctly re-derived during decryption. Ensure backwards compatibility with old ciphertexts using a try/except fallback block.

## 2026-07-09 - Missing Authorization and Rate Limiting on LLM Draft Generation Endpoints
**Vulnerability:** The LLM draft generation endpoints (`/api/snapshots/{uuid}/reversing-spec.md` and `/api/snapshots/{uuid}/index-design.md`) allowed any low-privilege `viewer` to repeatedly invoke heavy LLM processing without specific rate-limiting or elevated privileges.
**Learning:** Resource-intensive endpoints, particularly those invoking external APIs (like LLMs), are highly susceptible to DoS attacks and resource exhaustion. Relying solely on global API rate limits is insufficient. Furthermore, actions that generate complex output from database schemas should be restricted to appropriate roles.
**Prevention:** Always implement endpoint-specific rate limiting for resource-heavy operations (e.g., a limit of 10 requests per 60 seconds). Additionally, restrict execution to higher-privileged roles (like `editor` or `owner`) by explicitly verifying role requirements during the authorization step (`require_project_member(..., minimum_role="editor")`).

## 2026-07-08 - ECDSA Vulnerability and CI Failures
**Vulnerability:** The `ecdsa` package version `0.19.2` was flagged for a Minerva timing attack on P-256. This triggered the GitHub Actions dependency-review CI failure.
**Learning:** CI checks like `dependency-review` strictly evaluate both dependency locks (`requirements.lock`, `requirements-dev.lock`) against vulnerability databases. Adding a vulnerable version implicitly via indirect dependencies will fail the build.
**Prevention:** Pin the vulnerable dependency (`ecdsa<0.19.2`) explicitly in `pyproject.toml` or `requirements` files and recompile locks to prevent the inclusion of vulnerable transitive dependencies.

## 2026-07-08 - DSN Password Redaction Leakage via Encoding Variations
