...
## 2026-09-22 - [Optimize Prisma Schema Export: Replace nested array scans with pre-computed O(1) Maps]
**Learning:** During Prisma schema generation, the application iterated over all edges and columns, performing O(N) `.find()` searches on `node.data.columns` inside a loop, and another O(N) array inclusion check to find relationship definitions for every column. This results in O(N*C*E) complexity which degrades performance for large graphs.
**Action:** Pre-compute lookup Maps with composite keys (e.g., `sourceModel:sourceField`) before iterating through graph elements to reduce search complexity from O(N*C) to O(1) lookups, significantly improving export performance while maintaining the same output and 100% test coverage.
