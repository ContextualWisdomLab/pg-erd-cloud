...
## 2024-09-14 - Optimize `sanitizeHandleId` performance using `for...of` loop
**Learning:** In highly frequently called paths (like ERD handle ID generation), using `Array.from()` allocates unnecessary arrays, which increases garbage collection pressure. `for...of` loops iterate through Unicode characters properly without array allocation overhead.
**Action:** Prefer `for...of` loops over `Array.from()` for Unicode-aware string iteration in hot paths where minimizing allocation is important.
