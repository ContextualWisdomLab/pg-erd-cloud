...
## 2024-07-31 - React Flow Performance with Derived State
**Learning:** For React Flow performance during drag events (60fps updates), avoiding creating new `data` objects in derived state (like `visibleNodes`) is critical as it breaks `React.memo` and forces full re-renders of the entire graph on every micro-update.
**Action:** Use a `WeakMap` keyed by the stable `node.data` reference to cache decorated state and search results, preserving object identity across frames.
