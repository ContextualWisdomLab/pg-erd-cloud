...
## 2023-10-25 - Prevent O(N * C * E) performance bottlenecks in Prisma export
**Learning:** When iterating through columns of multiple tables to generate Prisma schema and checking relationship edges for each column, nesting loops causes O(N * C * E) time complexity. Additionally, using `.find()` inside a loop to check if a field is a primary key adds an O(C) complexity to the O(E) loop.
**Action:** Pre-compute lookup maps with composite keys (like `${sourceModel}:${sourceField}`) and `Set`s for primary keys before the main iterations to ensure scalable O(1) performance during generation.
