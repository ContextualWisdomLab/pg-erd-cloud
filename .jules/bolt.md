...
## 2024-08-15 - [Decode Hex Handles in ERD Export]
**Learning:** When rendering relationships in ERD exports, blindly replacing `src-` and `tgt-` handle prefixes leaks base hex encoded handles instead of actual column names. Decoding handles properly requires explicit helper utilities that unpack the React Flow ID safely.
**Prevention:** Always decode encoded UI identifiers before attempting to map them back to data fields.
