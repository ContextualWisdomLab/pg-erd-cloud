...
## 2024-07-25 - Avoid Array.from for Short Strings in Hot Paths
**Learning:** Using `Array.from` on strings for character iteration creates an intermediate array and increases garbage collection overhead. In hot paths (like encoding node IDs in ERD graphs), this causes unnecessary GC pressure.
**Action:** Replace `Array.from` with a `for...of` loop over the string to iterate directly over characters without creating intermediate arrays.
