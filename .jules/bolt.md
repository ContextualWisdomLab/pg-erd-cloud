...
## 2026-08-19 - Avoid Array.from for short strings on hot paths
**Status:** The separately invocable `frontend/scripts/handleUtils.benchmark.ts` records 50 deterministic counterbalanced pairs and a supporting 41.80% paired median elapsed-time improvement in `frontend/docs/benchmark_results/handleUtils.json`. Its `heapUsed` delta is diagnostic retained-heap evidence only, not an allocation measurement.
**Learning:** `Array.from(string)` materializes an intermediate array, while direct `for...of` iteration preserves ECMAScript string-iteration semantics and avoids that container. The bounded benchmark supports the implementation choice for this corpus and runtime; it does not generalize to all engines or claim reduced allocation.
**Action:** Keep the `for...of` implementation for this hot path, rerun the benchmark when the supported Node/V8 runtime changes, and remove the recommendation if output parity or the measured benefit no longer holds.
**Evidence:** ECMA International, *ECMAScript 2026 Language Specification*, Array.from and string iteration, https://262.ecma-international.org/17.0/; Georges, Buytaert, and Eeckhout, “Statistically rigorous Java performance evaluation,” *ACM SIGPLAN Notices* 42(10), 57–76 (2007), https://doi.org/10.1145/1297027.1297033; OpenJS Foundation, *Process—Node.js v26 documentation*, `process.memoryUsage`, https://nodejs.org/docs/latest-v26.x/api/process.html#processmemoryusage; V8 Project, “Orinoco: Young generation garbage collection,” https://v8.dev/blog/orinoco-parallel-scavenger. These sources support preserving iteration semantics, paired repeated measurements, interpreting `heapUsed` as an observation, and treating GC-sensitive effects cautiously.
