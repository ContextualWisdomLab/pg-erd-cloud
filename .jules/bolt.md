...
## 2024-07-20 - Avoid O(N^2) inline sum() during DBML import
**Learning:** In backend parsing logic, using inline generator expressions like `sum(1 for c in columns if c["relation_oid"] == oid)` inside loops creates an O(N^2) complexity, leading to severe performance issues for large databases during DBML imports.
**Action:** Replace nested generator expressions used for position tracking or occurrences with an auxiliary O(1) dictionary counter initialized outside the loop.
