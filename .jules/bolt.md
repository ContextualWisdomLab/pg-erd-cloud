...
## 2024-09-11 - Avoid Array.from for frequent short string manipulations
**Learning:** Using `Array.from` on strings creates short-lived intermediate array allocations. When called frequently, such as creating IDs for columns in large ERD diagrams, it leads to GC pressure.
**Action:** Replace `Array.from` with a `for...of` loop when iterating over short strings to perform per-character transformations, avoiding intermediate array allocation.
