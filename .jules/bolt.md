...
## 2024-05-18 - Prevent array allocation overhead in string iteration
**Learning:** Using `Array.from(string)` creates intermediate array allocations which increase garbage collection overhead. While fine for short strings normally, in hot paths like ERD graph node processing, this can add up.
**Action:** Prefer `for...of` loops over `Array.from()` for iterating through characters of a string when constructing new strings or executing operations in hot paths to prevent unnecessary allocations.
