...
## 2026-07-06 - Avoid intermediate array allocations in Set/Map initialization\n**Learning:** Creating `new Map()` or `new Set()` with `array.map()` leads to intermediate array allocations and unnecessary garbage collection overhead.\n**Action:** Use performant `for...of` loops and `.set()`/`.add()` methods when initializing Maps and Sets.
