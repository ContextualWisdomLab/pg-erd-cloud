...
## 2026-07-06 - Avoid array allocations for string building in loops
**Learning:** Using `.flatMap()`, array spread syntax (`...`), and `.join()` for building large strings inside React hooks iterating over ERD nodes creates unnecessary array allocations and increases garbage collection overhead.
**Action:** Use iterative string concatenation (`+=`) instead of array creation methods when building complex strings inside loops.
