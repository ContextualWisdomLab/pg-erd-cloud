...
## 2026-06-25 - Avoid O(N) Sets with Spread for Search String Normalization
**Learning:** Using `Array.from(new Set(search.trim().toLocaleLowerCase().split(/\s+/).filter(Boolean)))` executes multiple intermediate array allocations (`split`, `filter`, array mapping into Set, then Array from Set) for every search check.
**Action:** In search loops or general helpers, use simpler iteration or inline functions instead of large chain manipulations (like `new Set` with `.split().filter()`) when processing short strings, avoiding continuous memory allocation and garbage collection.
