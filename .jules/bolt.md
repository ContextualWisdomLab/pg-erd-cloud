...
## 2024-05-16 - [Replace Array.from with for...of in hot paths]
**Learning:** Using `Array.from` with a mapping function on short strings like column names creates intermediate arrays, which increases garbage collection pressure, particularly in hot paths like ERD graph processing where strings are encoded rapidly.
**Action:** Use `for...of` loops to iterate over strings and construct the result using string concatenation to avoid unnecessary array allocations in performance-critical sections.
