...
## 2024-05-24 - Optimize DSN query string parsing by caching secret key evaluation
**Learning:** Parsing DSNs often involves repeated checking of the same few URL parameter keys (e.g., `user`, `password`, `sslmode`) using expensive operations like `unquote_plus` and regular expression searches. These keys rarely change and the sets of potential keys used are quite small across the lifetime of the application. Furthermore, the vast majority of URL query parameter keys do not contain URL-encoded characters (`%` or `+`).
**Action:** Introduced a module-level `functools.lru_cache` for the `_is_secret_key` function to memoize the results of regex searches on query string keys. Additionally, implemented a fast-path within this function to skip the `unquote_plus` operation entirely if the key does not contain `%` or `+` characters. This led to a roughly 3x performance improvement in the hot path.
