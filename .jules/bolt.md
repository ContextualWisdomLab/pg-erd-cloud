...
## 2026-07-21 - Optimize React Flow 60fps Re-rendering latency via WeakMap caching
**Learning:** During 60fps React Flow node drag events, nodes are continuously recreated, but their `.data` properties maintain stable references. Any derived state computations mapping `nodes` (like dynamically injecting search highlights via object spreads `data: { ...node.data, isHighlighted }`) creates a new `data` reference on every render frame. This completely breaks React.memo fast-path checks (`prev.data === next.data`) in custom node components (like `TableNode`), forcing massive React tree re-renders and visible lag.
**Action:** When computing derived state over React Flow nodes in hot rendering paths, utilize a `WeakMap` keyed by the stable `node.data` reference to cache decorated state outputs (e.g. `cache.set(node.data, decoratedData)`). This preserves the underlying object identity across drag events, preserving memoization fast paths and reducing rendering latency.
