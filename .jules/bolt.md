...
## 2026-09-13 - Optimize O(N^2) Array Find loop in fkColumnsForEdge
**Learning:** Nested array `.find()` iterations within loops parsing graph connections for export (DDL generation) result in O(N^2) complexity and unnecessary callback allocations, degrading performance for large diagram outputs.
**Action:** Replaced repeated `.find()` calls on columns inside the edge iteration loop with a standard `for...of` loop with early `break` statements.
## 2026-09-13 - Optimize O(N^2) Array Find loop in Prisma Export
**Learning:** Nested array `.find()` iterations within loops parsing graph connections for export (Prisma schema generation) result in O(E * C) complexity and unnecessary callback allocations, degrading performance for large diagram outputs.
**Action:** Pre-compute O(1) Set of primary key columns for each node to avoid O(E * C) array scans during edge iteration.
