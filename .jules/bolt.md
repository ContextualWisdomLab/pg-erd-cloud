...
## 2024-07-26 - Avoid O(N^2) complexity with sum() generator in DBML parsing
**Learning:** During DBML parsing, calculating column positions with an inline generator `sum(1 for c in columns if c["relation_oid"] == oid) + 1` causes O(N^2) complexity, significantly degrading performance when processing schemas with thousands of columns.
**Action:** Replace inline generator scans over expanding lists with an O(1) auxiliary dictionary counter (`col_count_by_oid: dict[int, int]`) to maintain counters efficiently during single-pass iterations.
