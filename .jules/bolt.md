...
## 2024-05-18 - Avoid O(N * C * E) Complexity in Prisma Exporter
**Learning:** Using a nested loop over an array or map inside another loop over all nodes and columns, such as iterating `edgesProcessed` for every field, creates severe `O(N * C * E)` bottlenecks that slow down diagram export. Additionally, using array `.find()` inside the edges loop increases complexity unnecessarily.
**Action:** Pre-compute O(1) lookups using a `Map` with a composite key (e.g. `${modelName}:${fieldName}`) outside of the nodes loop to replace the nested edge iteration. Also, pre-compute metadata maps (like a set of primary keys per node) before iterating over edges to prevent redundant inner-loop array searches.
