...
## 2024-09-14 - Map cache for sanitizeHandleId
**Learning:** In React Flow, `sanitizeHandleId` using `Array.from` and `codePointAt` is extremely slow when called repeatedly during drag events. V8 struggles to optimize the array allocation per-character.
**Action:** Use a module-level `Map` to memoize `sanitizeHandleId` and avoid repeated array allocations. Add explicit comments outlining performance impact.
