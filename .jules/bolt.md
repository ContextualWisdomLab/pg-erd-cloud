...
## 2024-05-20 - [Handle ID Generation Bottleneck]
**Learning:** High-frequency operations in React Flow like edge rendering and graph exports can severely bottleneck on redundant string manipulations (e.g., `Array.from` with `codePointAt` for column names).
**Action:** Always memoize computationally expensive pure string operations inside tight loops or high-frequency render paths using a bounded `Map`-based LRU cache to prevent redundant string allocations and decoding overhead, ensuring the cache is properly encapsulated within a factory function.
