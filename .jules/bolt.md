...
## 2024-08-21 - Optimize Prisma Export Relation Lookups
**Learning:** Building relation mappings for Prisma exports by iterating over all edges for each column of each node (`O(N * C * E)`) causes severe UI lag for diagrams with many relations.
**Action:** Always pre-compute edge properties mapped to `model:field` composite keys using `Map` (`O(E)`) before looping, reducing relation lookup inside the column loop to an amortized `O(1)`.
**Research:** Fredman, M. L., Komlós, J., & Szemerédi, E. (1984). Storing a sparse table with O(1) worst case access time. *Journal of the ACM, 31*(3), 538–544. https://doi.org/10.1145/828.1884. The paper establishes constant-time lookup for a precomputed table, supporting this exporter’s one-time edge index followed by direct `model:field` lookups; the exporter still uses JavaScript’s amortized `Map` guarantee rather than claiming the paper’s worst-case bound.
