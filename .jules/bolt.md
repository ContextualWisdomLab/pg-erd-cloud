...
## 2024-07-28 - Optimize O(N) Array Finding with String Decoding
**Learning:** Found an O(N^2) bottleneck when exporting diagrams. Inside an O(E) loop over edges, we used `.find()` on the columns array (O(C)) and repeatedly invoked `sourceColumnHandleId` to encode each column name for comparison with the edge handle. This string formatting/allocation inside the inner loop was extremely expensive, pushing export times over 350ms for large graphs.
**Action:** Since we already know the target encoded string, we can implement an O(1) decoding function `parseColumnNameFromHandle` to retrieve the original column name *once* per edge, reducing the inner lookup from an expensive mapped `.find()` to a fast primitive `.some()` check (or a Map lookup).
