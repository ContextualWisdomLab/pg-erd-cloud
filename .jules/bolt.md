...
## 2026-09-17 - LRU Cache for High-Frequency String Parsing
**Learning:** Functions like `sanitizeHandleId` that use `Array.from` for unicode-safe string splitting are heavily called during React Flow renders and interactions. These per-character string allocations and garbage collections create measurable GC pressure when rendering complex diagrams.
**Action:** Always encapsulate frequently called pure functions within a bounded LRU cache (e.g. `Map` with `MAX_SIZE` eviction and `delete/set` re-insertion for LRU tracking) to drastically reduce main thread string allocation during layout and dragging.
## 2026-09-17 - LRU Cache Optimization Rejected
**Learning:** An optimization to use a bounded LRU Map for caching React Flow node rendering ID generation (`sanitizeHandleId` doing `Array.from` character conversions) was rejected and the PR was closed due to having "no effective or measurable impact" in practice. The optimization added complexity without proving sufficient performance gains over the engine's built-in string manipulation speed.
**Action:** Before optimizing seemingly expensive small utility functions like string manipulation in React renders, verify that they actually cause a measurable bottleneck. Small string ops (even `Array.from` Unicode parsing) are heavily optimized by V8, so manual caching (like LRU maps) might add GC/memory overhead that cancels out the CPU savings.
