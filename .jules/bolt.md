...
## 2024-08-04 - React Flow Drag Performance and WeakMap cache
**Learning:** React Flow의 60fps 드래그 이벤트 중 `visibleNodes`와 같이 새로 생성된 객체를 반환하는 매핑 작업은 커스텀 노드의 `React.memo` 얕은 비교(`prev.data === next.data`)를 깨뜨려, 전체 그래프에 대한 막대한 리렌더링을 유발합니다.
**Action:** `useMemo` 내부에 원본 `node.data` 참조를 키로 사용하는 `WeakMap` 캐시를 두어 파생된 상태 객체의 동일성을 보장함으로써, 드래그 등 잦은 상태 업데이트 시 전체 React 리렌더링을 효과적으로 생략할 수 있습니다.
