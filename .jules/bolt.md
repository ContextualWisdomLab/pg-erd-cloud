...
## 2026-07-08 - Optimize O(N^2) lookups to O(1) in autoInfer relationships
**Learning:** Nested array `.find()` iterations within loops parsing table schema parts and columns result in O(N^2) complexity, degrading ERD inference performance for large datasets.
**Action:** Replaced `Set` of table names and nested `nodes.find()` with a pre-computed `Map<string, Node>` (where key is table name), enabling O(1) node lookup and dropping overall complexity to O(N).
