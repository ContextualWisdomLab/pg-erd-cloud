...
## 2024-08-14 - Optimize Array.from for string iteration
**Learning:** Using `Array.from(string).join()` inside hot paths causes severe GC pressure due to intermediate array allocation.
**Action:** Replace `Array.from(string)` with an iterative loop and string concatenation to avoid allocating intermediate arrays when transforming strings on hot paths like identifier sanitization. This is verified by `frontend/src/erd/handleUtils.bench.ts`.
