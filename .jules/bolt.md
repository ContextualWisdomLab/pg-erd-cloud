...
## 2024-07-28 - Avoid O(N) Array Iteration when Parsing Handled Identifiers
**Learning:** During ERD column verification processes like `fkColumnsForEdge`, checking edge mappings using `.find()` alongside a `.replace()`-based encoding inside a loop over all column handles results in significant O(C) string allocation overhead per edge.
**Action:** Expose a centralized `parseColumnNameFromHandle` decoding utility instead, allowing extraction of the target string in O(1). Then use `.some()` to verify column existence natively, bypassing intermediate string re-encoding.
