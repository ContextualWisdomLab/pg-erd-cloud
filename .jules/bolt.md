...
## 2024-07-27 - Optimize Array .find() in ERD Export DDL
**Learning:** In `exportDDL`, finding source/target handle columns inside `fkColumnsForEdge` used repeated `O(N)` `Array.find()` calls inside the `edges` loop, creating `O(E * C)` complexity which degrades performance for large graphs.
**Action:** Precompute and cache `Map` structures mapping column handles to column names per node before evaluating edges, transforming the lookup to `O(1)` and significantly accelerating the DDL generation.
