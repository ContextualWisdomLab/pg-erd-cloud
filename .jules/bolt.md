...
## 2025-03-03 - O(1) Edge Handle Resolving in ERD Exports
**Learning:** Resolving column names from edge handles using O(N) array loops (like `.find(c => sourceColumnHandleId(c.column_name) === edge.sourceHandle)`) inside O(E) edge iteration causes severe O(E*C) bottlenecks and excessive GC allocations during string formatting.
**Action:** Implemented a new O(1) utility function `parseColumnNameFromHandle` in `frontend/src/erd/handleUtils.ts`. Use this to directly parse the string handle across ERD exports (DBML, DDL, Mermaid, Dictionary), avoiding repeated looping and encoding.
