...
## 2024-05-24 - ERD rendering hot path optimizations
**Learning:** In hot paths like frontend ERD graph processing, using `Array.from` for string manipulation causes excessive intermediate array allocations which increases GC pressure.
**Action:** Prefer `for...of` loops over `Array.from` for short string processing in hot paths to minimize allocations and reduce overhead.
