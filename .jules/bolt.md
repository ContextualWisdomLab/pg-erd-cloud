...
## 2025-02-12 - Decouple expensive search from node drag updates
**Learning:** React Flow generates new node position objects during dragging. Previously, `searchMatchedNodeIds` depended on the `nodes` array directly. Since dragging at 60fps causes continuous re-renders and updates to `nodes`, this triggered the O(N*C) search function over the entire graph continuously, causing UI lag.
**Action:** Decouple search logic from positional data by caching the search result via `useRef` and comparing `nodes[i].data` directly. Only recalculate when the internal node data (like title or columns) or the search query actually changes.
