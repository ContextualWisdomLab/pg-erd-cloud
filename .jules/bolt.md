...
## 2024-05-24 - Avoid intermediate allocations in ERD exports and lookups
**Learning:** Using `Array.from(string)` for character iteration, or `Array.find()` / `Set` allocations inside tight loops for relationship processing creates significant intermediate garbage, triggering GC pauses and performance regressions in large graph exports.
**Action:** Replace `Array.from(string)` with `for...of` loops. Replace `Array.find()` and dynamic `Set` creations in tight nested loops with explicit `for` loop scans and in-place additions to existing collections.
