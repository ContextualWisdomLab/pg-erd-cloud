...
## 2026-06-25 - Avoid Map allocations in frontend ERD loops & mutate asyncpg records in-place\n**Learning:** The frontend `snapshotToGraph` iterates over thousands of columns to generate the graph, creating new Maps and Sets in inner loops which caused high GC pressure. Mutating `asyncpg` dictionaries in-place in backend (`add_column_examples`) is safe for fresh snapshot data and significantly reduces memory allocation overhead.\n**Action:** Re-use references (e.g., check `if not in` instead of `setdefault`) to avoid executing expensive functions like `infer_column_example` for all columns. Avoid instantiating new arrays or sets repeatedly inside loops when aggregating relational data.
