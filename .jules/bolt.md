...
## 2024-07-21 - Optimize handle generation using for...of instead of Array.from
**Learning:** Using `Array.from(string)` in hot paths like ERD handle generation allocates intermediate arrays for each string transformation, creating significant garbage collection pressure during large graph renders.
**Action:** Replace `Array.from(string)` with a `for...of` loop to iterate over characters and build strings directly without allocating intermediate arrays.
