...
## 2024-07-28 - Avoid O(N*C*E) Complexity in Prisma Export Relation Lookups
**Learning:** A nested loop scanning an `O(E)` array inside a column loop (O(C)) which is inside a node loop (O(N)) leads to O(N * C * E) time complexity, causing serious bottlenecks during export of large ERDs.
**Action:** Pre-compute an `O(1)` nested `Map` mapping `SourceModel -> FieldName -> Relation` outside the main loop, reducing the complexity to O(E + N * C).
