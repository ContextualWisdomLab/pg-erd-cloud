...
## 2024-07-27 - O(N * C) String Allocation in Edge Handle Parsing
**Learning:** Extracting column names from edge handles in ERD exports (DBML, Dictionary, Mermaid) previously involved O(N * C) operations (iterating through all table columns and re-encoding their names with `sourceColumnHandleId` to find a match). This causes severe CPU and memory allocation bottlenecks for diagrams with many edges and tables.
**Action:** Use a direct parsing approach with a utility like `parseColumnNameFromHandle` (decoding the hex-encoded string directly) to turn handle-to-column resolution into an O(1) operation.
