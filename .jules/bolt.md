...
## 2024-05-24 - [Avoid Array.map() overhead in hot data conversion paths]
**Learning:** In highly frequent structural updates like React Flow graph generation, using `.map()` introduces O(N) callback execution overhead and unnecessary garbage collection pressure due to intermediate array allocations.
**Action:** Use pre-allocated arrays (`new Array(length)`) and standard `for` loops in hot data conversion paths (e.g., `snapshotToGraph`) to improve parsing performance.
