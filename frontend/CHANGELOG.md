...
## [Unreleased]
### Added
- **사이드바 생성 폼 키보드 제출**: 편집기의 새 프로젝트 및 새 연결 입력을 네이티브 `<form>` 제출 경로로 연결해 입력에서 Enter를 눌러 생성할 수 있게 했습니다. 생성 중 중복 요청과 유효하지 않은 입력은 기존 핸들러 guard가 계속 차단하며, 실제 Enter 이벤트 회귀 테스트를 추가했습니다.
