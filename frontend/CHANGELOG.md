...
## [Unreleased]
### Added
- **비활성 내보내기 작업의 발견 가능성과 시각 상태 유지**: 아직 사용할 수 없는 내보내기·접근 관리 버튼을 포커스 가능한 `aria-disabled="true"` 상태로 유지하면서, 공통 비활성 토큰으로 회색 시각 상태와 `not-allowed` 포인터 피드백을 보존하고 보이는 선행조건 설명으로 다음 조치를 안내합니다.
