...
### Security
- `nanoid` 트랜지티브 의존성을 잠금파일에서 `3.3.18`로 갱신해 GHSA-2v37-7h3g-55p8(High, `customAlphabet`/`customRandom`의 `size = 0` 무한 루프)을 제거했습니다. nanoid는 Vite/PostCSS 빌드 툴체인 전용 트랜지티브 의존성으로 배포 런타임 경로에 없으며, 조치 후 `npm audit` → High 이상 취약점 0건. 변경 범위는 `frontend/package-lock.json`이며, 애플리케이션 소스는 수정하지 않았습니다.
