...
describe('App orchestration coverage', () => {
  ...
  it('covers guarded editor actions, navigation callbacks, and form selectors', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Project', { exact: true }), { target: { value: 'p2' } })
    fireEvent.change(screen.getByLabelText('Connection', { exact: true }), { target: { value: 'c1' } })
    ...
    fireEvent.change(screen.getByLabelText('Project', { exact: true }), { target: { value: '' } })
    forceClick(screen.getByRole('button', { name: 'Save connection' }))
    fireEvent.change(screen.getByLabelText('Project', { exact: true }), { target: { value: 'p1' } })
    fireEvent.change(screen.getByLabelText(/New connection \(DSN\)/), { target: { value: ' ' } })
    forceClick(screen.getByRole('button', { name: 'Save connection' }))
    fireEvent.change(screen.getByLabelText('Connection', { exact: true }), { target: { value: '' } })
