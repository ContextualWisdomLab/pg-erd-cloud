...
vi.mock("./erd/autoInfer", () => ({
  inferRelationships: exports.inferRelationships,
}));
    ...
    return (
      <div data-testid="react-flow">
        ...
        <button
          ...
          onClick={() =>
            props.onConnect?.({ source: "table-1", target: "table-2" })
          }
        />
        <button
          type="button"
          ...
          onClick={(event) =>
            props.onEdgeClick?.(event, props.edges[0] ?? edge)
          }
        />
        <button
          type="button"
          data-testid="flow-edge-unlabeled"
          onClick={(event) =>
            props.onEdgeClick?.(event, { ...edge, label: undefined })
          }
        />
        <button
          type="button"
          ...
          onDoubleClick={(event) =>
            props.onNodeDoubleClick?.(event, props.nodes[0] ?? initialNode)
          }
        />
        {props.children}
      </div>
...
vi.mock("./components/modals", () => ({
  AddTableModal: (props: any) => (
    <div data-testid="add-modal" data-open={props.isOpen}>
      <button
        ...
        onClick={props.onAddTableSubmit}
      />
      {props.isOpen ? (
        <>
          <button
            ...
            onClick={() => props.setNewTableName("audit_log")}
          />
          <button
            type="button"
          ...
          <button
            ...
            onClick={props.onAddTableCancel}
          />
        </>
      ) : null}
    </div>
  ...
  EditEdgeModal: (props: any) => (
    <div data-testid="edge-modal" data-open={Boolean(props.editingEdge)}>
      <button
        type="button"
        data-testid="edge-guard-submit"
        onClick={props.onRelSubmit}
      />
      <button
        type="button"
        data-testid="edge-guard-delete"
        onClick={props.onRelDelete}
      />
      {props.editingEdge ? (
        <>
          <button
            ...
            onClick={() => props.setRelLabel(" fk_new ")}
          />
          <button
            type="button"
      ...
      {props.isOpen ? (
        <>
          <button
            ...
            onClick={props.onCloseExport}
          />
          <button
            type="button"
            data-testid="export-copy-ddl"
            onClick={props.onCopyExportDdl}
          />
          <button
            type="button"
          ...
          <button
            type="button"
            data-testid="export-uml"
            onClick={props.onDownloadUml}
          />
          <button
            type="button"
            data-testid="export-mermaid"
            onClick={props.onDownloadMermaid}
          />
          <button
            type="button"
            data-testid="export-dbml"
            onClick={props.onDownloadDbml}
          />
          <button
            type="button"
            data-testid="export-csv"
            onClick={props.onExportDictionaryCsv}
          />
          <button
            type="button"
            data-testid="export-md"
            onClick={props.onExportDictionaryMarkdown}
          />
          <button
            type="button"
  ...
  GroupModal: (props: any) => (
    <div data-testid="group-modal" data-open={props.isOpen}>
      <button
        type="button"
        data-testid="group-create-guard"
        onClick={props.onCreateBusinessGroup}
      />
      {props.isOpen ? (
        <>
          <button
            ...
            onClick={() => props.setNewGroupName("Billing")}
          />
          <button
            type="button"
          ...
          <button
            type="button"
            data-testid="group-assign-missing"
            onClick={() =>
              props.onAssignBusinessGroup(
                props.nodes[0]?.id ?? "missing-node",
                "missing",
              )
            }
          />
          <button
            type="button"
            data-testid="group-assign"
            onClick={() =>
              props.onAssignBusinessGroup(
                props.nodes[0]?.id ?? "missing-node",
                props.businessGroups[0]?.id ?? "",
              )
            }
          />
          <button
            ...
            onClick={() =>
              props.onDeleteBusinessGroup(
                props.businessGroups[0]?.id ?? "missing",
              )
            }
          />
          <button
            ...
            onClick={props.onCloseGroupManager}
          />
        </>
      ) : null}
    </div>
  ...
  CardinalityModal: (props: any) => {
    ...
    return (
      <div data-testid="cardinality-modal" data-open={props.isOpen}>
        ...
        <span data-testid="card-strength-recommended">
          {props.strengthLabel("recommended")}
        </span>
        <span data-testid="card-strength-consider">
          {props.strengthLabel("consider")}
        </span>
        <span data-testid="card-strength-skip">
          {props.strengthLabel("skip")}
        </span>
        <button
          type="button"
          data-testid="card-skip-guard"
          onClick={() =>
            props.onApplyCardinalityRecommendation({
              ...recommendation,
              strength: "skip",
            })
          }
        ...
        {props.isOpen ? (
          <>
            <button
              type="button"
              data-testid="card-table-missing"
              onClick={() => props.onCardinalityTableChange("missing")}
            />
            <button
              type="button"
              data-testid="card-table"
              onClick={() => props.onCardinalityTableChange("table-2")}
            />
            <button
              type="button"
              data-testid="card-toggle"
              onClick={() => props.onCardinalityColumnToggle("email", true)}
            />
            <button
              type="button"
              data-testid="card-distinct-invalid"
              onClick={() =>
                props.onCardinalityDistinctCountChange("email", "bad")
              }
            />
            <button
              type="button"
              data-testid="card-distinct"
              onClick={() =>
                props.onCardinalityDistinctCountChange("email", "50")
              }
            />
            <button
              type="button"
            ...
            <button
              type="button"
              data-testid="card-apply-duplicate"
              onClick={() =>
                props.onApplyCardinalityRecommendation(recommendation)
              }
            />
            <button
              type="button"
              data-testid="card-apply-no-columns"
              onClick={() =>
                props.onApplyCardinalityRecommendation({
                  ...recommendation,
                  columns: undefined,
            ...
            <button
              type="button"
              data-testid="card-apply-second"
              onClick={() =>
                props.onApplyCardinalityRecommendation({
                  ...recommendation,
                  index_name: "idx_users_second",
                  columns: ["id"],
                })
              }
            />
            ...
            <button
              ...
              onClick={props.onCloseCardinalityWizard}
            />
          </>
        ) : null}
      </div>
  ...
  EditTableModal: (props: any) => (
    <div data-testid="table-modal" data-open={props.isOpen}>
      <button
        type="button"
        data-testid="table-delete-guard"
        onClick={props.onDeleteTable}
      />
      <form
        data-testid="table-submit-guard"
        onSubmit={props.onEditTableSubmit}
      />
      {props.isOpen && props.editingNode ? (
        <>
          ...
          <button
            ...
            onClick={props.onEditTableCancel}
          />
        </>
      ) : null}
    </div>
...
beforeEach(() => {
  ...
  api.createProject.mockResolvedValue({
    project_space_uuid: "p3",
    project_name: "New",
  });
  api.createConnection.mockResolvedValue({
    db_connection_uuid: "c2",
    conn_name: "New DB",
  });
  ...
  api.createShareLink.mockResolvedValue({
    url: "http://localhost/api/share/one",
  });
...
describe("App orchestration coverage", () => {
  ...
  it("navigates dashboard, project, and diagram states including empty/search branches", async () => {
    ...
    expect(
      screen.getAllByText("&lt;Billing &amp; Core&gt;").length,
    ).toBeGreaterThan(0);
    ...
    expect(
      screen.getByRole("heading", { name: "다이어그램" }),
    ).toBeInTheDocument();
    fireEvent.change(screen.getByLabelText("다이어그램 검색"), {
    ...
    expect(
      screen.getByRole("toolbar", { name: "ERD 캔버스 도구" }),
    ).toBeInTheDocument();
  ...
  it("creates projects, validates and creates connections, and starts a snapshot", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Schema filter (optional)"), {
      target: { value: " public " },
    });
    fireEvent.click(
      screen.getByRole("button", { name: "Reverse engineer → snapshot" }),
    );
    await waitFor(() =>
  ...
  it("polls a terminal snapshot, builds graph state, and exercises editor handlers", async () => {
    ...
    fireEvent.change(screen.getByLabelText("테이블 또는 컬럼 검색"), {
      target: { value: "users" },
    });
    expect(
      screen.getByText("1개 테이블 일치", { exact: false }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("되돌렸습니다", { exact: false }),
    ).toBeInTheDocument();
  ...
  it("adds nodes and exercises groups, cardinality, exports, inference, and clearing", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "인덱스 카디널리티 계산" }),
    );
    expect(screen.getByTestId("card-format")).toHaveTextContent("50%");
    expect(screen.getByTestId("card-strength-recommended")).toHaveTextContent(
      "추천",
    );
    expect(screen.getByTestId("card-strength-consider")).toHaveTextContent(
      "검토",
    );
  ...
  it("covers guarded editor actions, navigation callbacks, and form selectors", async () => {
    ...
    fireEvent.change(screen.getByLabelText("New connection (DSN)"), {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /Billing.*다이어그램 보기/ }),
    );
    expect(
      screen.getByRole("heading", { name: "다이어그램" }),
    ).toBeInTheDocument();
    ...
    fireEvent.change(screen.getByLabelText("새 프로젝트 이름"), {
      target: { value: "Roadmap" },
    });
    fireEvent.click(screen.getByRole("button", { name: "새 프로젝트" }));
    await waitFor(() =>
      expect(api.createProject).toHaveBeenCalledWith("Roadmap"),
    );
    fireEvent.click(screen.getAllByRole("button", { name: "열기" })[0]!);
    expect(
      screen.getByRole("heading", { name: "다이어그램" }),
  ...
  it("logs auto-layout failures and preserves nodes added after the undo snapshot", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText("정렬에 실패했습니다. 다시 시도해 주세요.", {
          exact: false,
        }),
      ).toBeInTheDocument(),
    );
  ...
  it("ignores stale project metadata failures after changing projects", async () => {
    ...
    expect(
      screen.queryByText(/stale (connections|snapshots)/),
  ...
  it("ignores duplicate share creation while a request is pending", async () => {
    ...
    api.createShareLink.mockReturnValueOnce(
      new Promise((resolve) => {
    ...
    await act(async () =>
      resolveShare({ url: "http://localhost/api/share/done" }),
    );
  });
      ...
      nodes: [
        {
          ...
          type: "tableNode",
          position: { x: 0, y: 0 },
          data: { columns: [], badges: { pk: false, fk: false } },
        },
        {
          id: "a-node",
          type: "tableNode",
  ...
  it("reports API effect failures, snapshot polling failures, share failures, and clipboard failures", async () => {
    ...
    fireEvent.click(screen.getByTestId("share-create"));
    await waitFor(() =>
      expect(screen.getByTestId("share-error")).toHaveTextContent("share down"),
    );

    api.createShareLink.mockResolvedValueOnce({
      url: "http://localhost/api/share/fail-copy",
    });
    fireEvent.click(screen.getByTestId("share-create"));
    await waitFor(() =>
      expect(screen.getByTestId("share-url")).toHaveTextContent("fail-copy"),
    );
