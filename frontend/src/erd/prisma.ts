...
function columnNameFromHandle(handle: string | null | undefined, prefix: string): string | null {
  if (!handle?.startsWith(prefix)) return null;

  const suffix = handle.slice(prefix.length);
  return decodeHandleId(suffix) ?? suffix;
}
...
export function exportPrisma(
  ...
  // ⚡ Bolt: Pre-compute edge relations by model+field to turn O(N*C*E) nested loops into O(1) lookups
  const outgoingRelationsByField = new Map<string, { targetModel: string, targetField: string, relationName: string }>();
  ...
  for (const edge of edges) {
    ...
    let sourceField = "";
    const sourceColumn = columnNameFromHandle(edge.sourceHandle, "src-");
    ...
    const targetColumn = columnNameFromHandle(edge.targetHandle, "tgt-");
    ...
    if (sourceField) {
      ...
      outgoingRelationsByField.set(`${sanitizeName(sourceNode.data.title)}:${sanitizeName(sourceField)}`, {
  ...
  for (const node of nodes) {
    ...
    for (const col of node.data.columns) {
      ...
      let relationDef = "";
      // ⚡ Bolt: O(1) lookup replaces O(E) array scan per column
