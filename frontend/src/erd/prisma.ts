...
export function exportPrisma(
  ...
  const outgoingRelationsByModelAndField = new Map<string, { targetModel: string, targetField: string, relationName: string }>();
  ...
  for (const edge of edges) {
    ...
    if (sourceField) {
      ...
      const srcModel = sanitizeName(sourceNode.data.title);
      const srcField = sanitizeName(sourceField);
      outgoingRelationsByModelAndField.set(`${srcModel}:${srcField}`, {
  ...
  for (const node of nodes) {
    ...
    for (const col of node.data.columns) {
      ...
      let relationDef = "";
      const edgeInfo = outgoingRelationsByModelAndField.get(`${modelName}:${fieldName}`);
      if (edgeInfo) {
