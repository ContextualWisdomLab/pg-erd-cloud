...
export function exportPrisma(
  ...
  const edgesProcessedByField = new Map<string, { sourceModel: string, targetModel: string, sourceFields: string[], targetFields: string[], relationName: string }>();

  // Pre-compute PK columns for O(1) lookup
  ...
  for (const node of nodes) {
    ...
    pkColumnsByNodeId.set(node.id, pkSet);
  }
  ...
  for (const edge of edges) {
    ...
    if (sourceField) {
      const isUnique = pkColumnsByNodeId.get(sourceNode.id)?.has(sourceField) || false;
      ...
      edgesProcessedByField.set(`${sModel}:${sField}`, {
        sourceModel: sModel,
  ...
  for (const node of nodes) {
    ...
    for (const col of node.data.columns) {
      ...
      let relationDef = "";
      // ⚡ Bolt: O(1) relation lookup instead of iterating all edges for every column in every node (O(N * C * E) -> O(1))
