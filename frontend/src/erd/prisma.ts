...
export function exportPrisma(
  ...
  const edgesProcessedBySource = new Map<string, { targetModel: string, targetFields: string[], relationName: string }>();
  ...
  for (const edge of edges) {
    ...
    if (sourceField) {
      ...
      edgesProcessedBySource.set(`${sanitizeName(sourceNode.data.title)}.${sanitizeName(sourceField)}`, {
