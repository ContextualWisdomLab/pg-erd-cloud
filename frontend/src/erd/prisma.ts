...
export function exportPrisma(
  ...
  // ⚡ Bolt: Use O(1) Map for edge lookups instead of iterating over edgesProcessed repeatedly.
  // Key format: `${modelName}:${fieldName}`
  ...
  for (const edge of edges) {
    ...
    if (sourceField) {
      // ⚡ Bolt: O(C) standard loop instead of Array.find() to avoid allocating callback functions
      let isUnique = false;
      for (let i = 0; i < sourceNode.data.columns.length; i++) {
        if (sourceNode.data.columns[i].column_name === sourceField) {
          isUnique = sourceNode.data.columns[i].is_pk;
          break;
        }
      }
      ...
      relationsBySourceField.set(`${sourceModel}:${sanitizeName(sourceField)}`, {
  ...
  for (const node of nodes) {
    ...
    for (const col of node.data.columns) {
      ...
      let relationDef = "";
      // ⚡ Bolt: O(1) lookup using relationsBySourceField instead of O(E) loop over all edges
