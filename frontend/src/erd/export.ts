...
export function exportDDL(nodes: Node<TableNodeData>[], edges: Edge[]): string {
  ...
  for (const edge of edges) {
    ...
    if (sourceNode && targetNode) {
      const constraintName = edge.label
        ? edge.label
        : `fk_${edge.source}_${edge.target}`;
      const sourceTable = quoteSqlIdentifier(
        sourceNode.data.title || sourceNode.id,
      );
      const targetTable = quoteSqlIdentifier(
        targetNode.data.title || targetNode.id,
      );
...
function plantText(value: unknown): string {
  return escapeXml(value).replace(PLANT_TEXT_ESCAPE_RE, (char) =>
    char === "\\" ? "\\\\" : " ",
  );
}

function indexesByRelation(
  snapshot?: SnapshotJson | null,
): Map<string, SnapshotJson["indexes"]> {
...
function indexLabel(ix: DisplayIndex): string {
  const extensions =
    "operator_class_extensions" in ix
      ? new Set(
          [
            ix.access_method_extension,
            ...(ix.operator_class_extensions || []),
          ].filter(Boolean),
        )
      : new Set<string>();
...
function columnLabel(col: TableNodeData["columns"][number]): string {
  ...
  const example =
    col.example_value === null || col.example_value === undefined
      ? ""
...
function tableLabel(node: Node<TableNodeData>): string {
  ...
  const group = node.data.businessGroup
    ? ` [${node.data.businessGroup.name}]`
    : "";
...
export function exportPlantUml(
  ...
  for (const node of nodes) {
    lines.push(
      `entity "${plantText(tableLabel(node))}" as ${plantAlias(node.id)} {`,
    );
    for (const col of node.data.columns || []) {
      lines.push(
        `  ${col.is_pk ? "*" : ""}${plantText(columnLabel(col))} : ${plantText(col.data_type)}${col.is_not_null ? " <<not null>>" : ""}`,
      );
    }
    for (const ix of [
      ...(indexes.get(node.id) || []),
  ...
  for (const edge of edges) {
    ...
    lines.push(
      `${plantAlias(edge.source)} --> ${plantAlias(edge.target)}${label}`,
    );
  }
...
export function exportDiagramSvg(
  ...
  const heights = new Map(
    nodes.map((node) => {
      ...
      const indexRows = Math.min(
        (indexes.get(node.id)?.length || 0) + (node.data.indexes?.length || 0),
        8,
      );
      return [
        node.id,
  ...
  for (const edge of edges) {
    ...
    const sy =
      source.position.y +
      offsetY +
      (heights.get(source.id) || headerHeight) / 2;
    const tx = target.position.x + offsetX;
    const ty =
      target.position.y +
      offsetY +
      (heights.get(target.id) || headerHeight) / 2;
    const mx = (sx + tx) / 2;
    parts.push(
      `<path d="M ${sx} ${sy} C ${mx} ${sy}, ${mx} ${ty}, ${tx} ${ty}" fill="none" stroke="#64748b" stroke-width="1.5" marker-end="url(#arrow)"/>`,
    );
    if (edge.label) {
      parts.push(
        `<text x="${mx}" y="${(sy + ty) / 2 - 4}" font-family="system-ui, sans-serif" font-size="11" fill="#475569">${escapeXml(edge.label)}</text>`,
      );
    }
  }
  ...
  for (const node of nodes) {
    ...
    const groupColor = node.data.businessGroup
      ...
      : "#e0f2fe";
    parts.push(
      `<rect x="${x}" y="${y}" width="${width}" height="${height}" rx="8" fill="#fff" stroke="#cbd5e1"/>`,
    );
    parts.push(
      `<rect x="${x}" y="${y}" width="${width}" height="${headerHeight}" rx="8" fill="${escapeXml(groupColor)}" fill-opacity="${node.data.businessGroup ? "0.18" : "1"}" stroke="#cbd5e1"/>`,
    );
    if (node.data.businessGroup) {
      parts.push(
        `<rect x="${x}" y="${y}" width="6" height="${height}" rx="3" fill="${escapeXml(groupColor)}"/>`,
      );
    }
    parts.push(
      `<text x="${x + 12}" y="${y + 22}" font-family="system-ui, sans-serif" font-size="13" font-weight="700" fill="#0f172a">${escapeXml(tableLabel(node))}</text>`,
    );
    let rowY = y + headerHeight + 16;
    for (const col of node.data.columns || []) {
      parts.push(
        `<text x="${x + 12}" y="${rowY}" font-family="ui-monospace, monospace" font-size="11" fill="#111827">${col.is_pk ? "* " : ""}${escapeXml(columnLabel(col))}: ${escapeXml(col.data_type)}${col.is_not_null ? " not null" : ""}</text>`,
      );
      rowY += rowHeight;
    }
    ...
    if (tableIndexes.length > 0) {
      parts.push(
        `<text x="${x + 12}" y="${rowY}" font-family="system-ui, sans-serif" font-size="11" font-weight="700" fill="#475569">Indexes</text>`,
      );
      rowY += rowHeight;
    }
    for (const ix of tableIndexes.slice(0, 8)) {
      parts.push(
        `<text x="${x + 12}" y="${rowY}" font-family="ui-monospace, monospace" font-size="11" fill="#334155">${escapeXml(indexLabel(ix))}</text>`,
      );
      rowY += rowHeight;
    }
  }
