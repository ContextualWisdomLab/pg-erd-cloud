...
function fkColumnsForEdge(
  ...
  const sourceHandleColumn = parsedSource && (sourceNode.data.columns || []).some(c => c.column_name === parsedSource) ? parsedSource : undefined;
  const targetHandleColumn = parsedTarget && (targetNode.data.columns || []).some(c => c.column_name === parsedTarget) ? parsedTarget : undefined;
