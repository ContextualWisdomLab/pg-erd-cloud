...
import { sourceColumnHandleId, targetColumnHandleId, parseHandleId } from './handleUtils';
...
function fkColumnsForEdge(
  ...
  const sourceHandleColumn = parseHandleId(edge.sourceHandle, 'src-');
  const targetHandleColumn = parseHandleId(edge.targetHandle, 'tgt-');
