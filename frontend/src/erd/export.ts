...
function fkColumnsForEdge(
  ...
  // Bolt: Parse handle directly in O(1) instead of iterating through nodes array
  // with re-encoding string match (O(N) operation per edge lookup).
  // We still check for existence in the underlying array for safety.
