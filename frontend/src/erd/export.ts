...
function fkColumnsForEdge(
  ...
  const sourceHandleColumn = parsedSource !== null && (sourceNode.data.columns || []).some(c => c && c.column_name === parsedSource) ? parsedSource : undefined;
  const targetHandleColumn = parsedTarget !== null && (targetNode.data.columns || []).some(c => c && c.column_name === parsedTarget) ? parsedTarget : undefined;
