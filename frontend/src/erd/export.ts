...
function fkColumnsForEdge(
  ...
  if (parsedSource && parsedTarget) {
    const sourceHandleColumn = (sourceNode.data.columns || []).some((c) => c.column_name === parsedSource) ? parsedSource : null;
    const targetHandleColumn = (targetNode.data.columns || []).some((c) => c.column_name === parsedTarget) ? parsedTarget : null;
