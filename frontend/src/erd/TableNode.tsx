    ...
    >
      ...
      <div className="tableNode__title">
        ...
        <span style={{ display: "inline-flex", gap: 6 }}>
          ...
          {data.badges?.pk ? (
            <span className="tableNode__badge" aria-label="Primary Key">PK</span>
          ) : null}
          {data.badges?.fk ? (
            <span className="tableNode__badge" aria-label="Foreign Key">FK</span>
          ) : null}
        </span>
      </div>
        ...
        {data.columns.slice(0, MAX_RENDERED_COLUMNS).map((c) => {
          ...
          return (
            <div key={c.column_name} className="tableNode__col">
              ...
              {c.is_pk ? <span className="tableNode__badge" aria-label="Primary Key">PK</span> : null}
              {c.is_not_null ? (
                <span className="tableNode__badge" aria-label="Not Null">NOT NULL</span>
              ) : null}
              <Handle
                type="source"
