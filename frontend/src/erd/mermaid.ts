...
export function exportMermaid(
  ...
  for (const edge of edges) {
    if (edge.sourceHandle) {
      ...
      if (parsedSource) fkNodeColumnPairs.add(`${edge.source}:${parsedSource}`);
    } else {
    ...
    if (edge.targetHandle) {
      ...
      if (parsedTarget) fkNodeColumnPairs.add(`${edge.target}:${parsedTarget}`);
    }
  }
  ...
  for (const node of nodes) {
    ...
    for (const col of node.data.columns) {
      ...
      // ⚡ Bolt: O(1) lookups using actual column name string instead of encoding every column
      const isFk =
