...
export function inferRelationships(nodes: Node<TableNodeData>[]): Edge[] {
  ...
  // ⚡ Bolt: Pre-compute node lookups by their base table name to avoid O(N^2) complexity
  // inside the nested column iteration loop when inferring relationships in large ERDs.
      ...
      if (colName.endsWith("_id")) {
        ...
        if (targetTableName && targetTableName !== srcTableName) {
          ...
          if (targetNode) {
            ...
            let targetColName = "";
            const idCol = targetNode.data.columns.find(
              (c) => c.column_name === "id",
            );
            if (idCol) {
              targetColName = "id";
            } else {
              const pkCol = targetNode.data.columns.find((c) => c.is_pk);
              if (pkCol) {
