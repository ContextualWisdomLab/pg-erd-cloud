...
describe('exportPrisma', () => {
  ...
  it('does not infer a relation from an unparsed source handle', () => {
    const nodes: Node<TableNodeData>[] = [
      {
        id: '1',
        position: { x: 0, y: 0 },
    ...
    expect(result).toContain('model accounts');
    expect(result).not.toContain('events_accounts');
  });

  it('emits a singular back-relation when the source column is a primary key', () => {
    const nodes: Node<TableNodeData>[] = [
      {
        id: '1',
        position: { x: 0, y: 0 },
        data: {
          title: 'profiles',
          badges: { pk: true, fk: true },
          columns: [{ column_name: 'user_id', data_type: 'integer', is_pk: true, is_not_null: true }],
        },
      },
      {
        id: '2',
    ...
    const result = exportPrisma(nodes, [{
      ...
      sourceHandle: 'src-user_id',
      targetHandle: 'tgt-id',
      label: 'profiles_users',
    }]);

    expect(result).toContain('profiles_user_id profiles? @relation("profiles_users")');
  });
