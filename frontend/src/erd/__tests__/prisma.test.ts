...
describe('exportPrisma', () => {
  ...
  it('handles optional sourceField when edge lacks sourceHandle', () => {
    const nodes: Node<TableNodeData>[] = [
      {
        id: '1',
        position: { x: 0, y: 0 },
    ...
    const result = exportPrisma(nodes, edges);
    // Since there is no sourceField, the relation field shouldn't be added to the nodes.
    expect(result).not.toContain('@relation("users_posts"');
  });

  it('handles unique relations to render optional type suffix', () => {
    const nodes: Node<TableNodeData>[] = [
      {
        id: '1',
        position: { x: 0, y: 0 },
      ...
      },
      {
        ...
        data: {
          ...
          columns: [
            { column_name: 'id', data_type: 'serial', is_pk: true, is_not_null: true },
            { column_name: 'user_id', data_type: 'integer', is_not_null: true, is_pk: true }, // unique (pk)
          ],
        },
      },
    ];
    ...
    const edges: Edge[] = [
      {
        ...
        sourceHandle: 'src-user_id',
        targetHandle: 'tgt-id',
        label: 'users_user_profiles',
      },
    ];

    const result = exportPrisma(nodes, edges);
    expect(result).toContain('user_profiles_user_id user_profiles? @relation("users_user_profiles")');
  });
});
