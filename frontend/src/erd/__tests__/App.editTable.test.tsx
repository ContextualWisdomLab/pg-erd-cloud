...
describe('App edit functionality', () => {
  ...
  it('renders compact visual labels while preserving toolbar accessible names', async () => {
    ...
    const exportButtons = [
      toolbarQueries.getByRole('button', { name: '이미지 내보내기 모달 열기' }),
      toolbarQueries.getByRole('button', { name: 'UML 내보내기 모달 열기' }),
      toolbarQueries.getByRole('button', { name: 'JSON 내보내기 모달 열기' })
    ];
