...
async def test_parse_snowflake_dsn_invalid():
    ...
    with pytest.raises(ValueError, match="Snowflake DSN must include an account identifier"):
    ...
    with pytest.raises(ValueError, match="Snowflake DSN must include a database path segment"):
        await _parse_snowflake_dsn("snowflake://user:pass@acct")
    with pytest.raises(ValueError, match="Snowflake DSN path must be /database or /database/schema"):
        await _parse_snowflake_dsn("snowflake://user:pass@acct/db/schema/extra")
    with pytest.raises(ValueError, match="Snowflake DSN query parameter is blank: warehouse"):
...
def test_connect(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="Snowflake reverse engineering requires the optional"):
        _connect()
...
def test_format_snowflake_data_type():
    ...
    assert _format_snowflake_data_type({"data_type": "NUMBER", "numeric_precision": 10}) == "NUMBER(10,0)"

def test_table_key():
    assert _table_key({"table_schema": "SCHEMA", "table_name": "TABLE"}) == ("SCHEMA", "TABLE")
    assert _table_key({}) == ("", "")
...
def test_constraint_def():
    ...
    assert _constraint_def("f", ["col1"], "SCHEMA", "TABLE", ["ref_col"]) == 'FOREIGN KEY ("col1") REFERENCES "SCHEMA"."TABLE" ("ref_col")'
...
def test_introspect_snowflake_sync_with_config_missing_schema_filter(monkeypatch):
    ...
    def fake_connect(**kwargs): return FakeConnection()
    ...
    monkeypatch.setattr("app.snowflake_introspect.introspect._fetch_dicts", fake_fetch_dicts)

    config = SnowflakeDsnConfig(
        account="acct", user="user", password=None, database="db", schema=None, warehouse=None, role=None, authenticator=None
    )
