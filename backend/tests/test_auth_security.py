...
def test_validate_jwt_header_crit():
    ...
    assert exc_info.value.status_code == 401
    assert "unsupported critical parameter: b64" in str(exc_info.value.detail)


def test_validate_jwt_header_crit_not_list():
    with pytest.raises(HTTPException) as exc_info:
        auth._validate_jwt_header({"alg": "RS256", "crit": "b64"})
    assert exc_info.value.status_code == 401
    assert "invalid crit header" in str(exc_info.value.detail)


def test_validate_jwt_header_crit_too_many():
    with pytest.raises(HTTPException) as exc_info:
        auth._validate_jwt_header({"alg": "RS256", "crit": [str(i) for i in range(11)]})
    assert exc_info.value.status_code == 401
    assert "too many crit parameters" in str(exc_info.value.detail)
