...
def test_annotation_names_reject_control_characters() -> None:
    with pytest.raises(ValidationError):
        TableAnnotationUpsertIn(schema_name="my\x00schema", relation_name="tbl", body="test")
    with pytest.raises(ValidationError):
        TableAnnotationUpsertIn(schema_name="sch", relation_name="my\ntbl", body="test")
