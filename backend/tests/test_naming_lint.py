...
def _snap(tables):
    ...
    for oid, (t, cols) in enumerate(tables.items(), start=1):
        relations.append(
            {"relation_oid": oid, "schema_name": "public", "relation_name": t}
        )
        for c in cols:
...
def test_flags_identifier_requiring_quotes():
    report = lint_naming(
        _snap({"MyTable": ["id"], "member": ["first-name", "2fa_flag"]})
    )
    cats = _cats(report)
    assert ("requires_quoting", "high") in cats
    targets = {
        i["target"] for i in report["items"] if i["category"] == "requires_quoting"
    }
