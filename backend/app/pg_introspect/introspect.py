...
_TLS_FILE_QUERY_KEYS = frozenset(
...
async def _connect_guarded_postgres(
    ...
    query_keys = {
        key.lower()
        for key, _value in parse_qsl(urlparse(dsn).query, keep_blank_values=True)
    }
    if query_keys & _TLS_FILE_QUERY_KEYS:
        raise ValueError("PostgreSQL DSN TLS file parameters are not supported")
