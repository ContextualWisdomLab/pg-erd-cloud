...
async def export_shared_snapshot_reversing_spec(
    ...
    if mode == "llm-draft":
        try:
            return await generate_reversing_llm_draft(_redact_sensitive_snapshot_fields(data.snapshot_json))
    ...
    return generate_reversing_spec(_redact_sensitive_snapshot_fields(data.snapshot_json), mode=mode)
...
async def export_shared_snapshot_index_design(
    ...
    if mode == "llm-draft":
        try:
            return await generate_index_design_llm_draft(_redact_sensitive_snapshot_fields(data.snapshot_json))
        except LlmConfigurationError as exc:
            raise HTTPException(
                status_code=503, detail="LLM configuration error"
            ) from exc
    ...
    return generate_index_design_spec(_redact_sensitive_snapshot_fields(data.snapshot_json), mode=mode)
