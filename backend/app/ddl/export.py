...
def _render_table_pg(
    ...
    if kind == "p" and not isinstance(partition_key, str):
        lines.append(
            f"-- NOTE: {_qname(schema, name)} is partitioned; "
            "partition definition was not available in this snapshot export"
        )
...
def _snapshot_json_to_postgresql_sql(snapshot: dict) -> str:
    ...
    This is conservative forward engineering (export):
    ...
    Limitations: partitioning clauses and some table options are reconstructed
    only when present in the snapshot.
    """
