...
def _map_type(
    data_type: str, table: list[tuple[re.Pattern[str], str]], default: str
) -> str:
...
def _column_names(cols: list[dict[str, Any]]) -> dict[str, str]:
    return {
        str(col.get("column_name")): _snake_name(col.get("column_name"), "column")
        for col in cols
    }
...
def _index(snapshot: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for pk in snapshot.get("pk_columns") or []:
        pk_by_oid.setdefault(pk.get("relation_oid"), set()).add(
            str(pk.get("column_name"))
        )
...
def generate_sqlalchemy_models(snapshot: dict[str, Any] | None) -> str:
    ...
    for rel in ix["relations"]:
        ...
        for col in cols:
            ...
            comment = (
                ""
                if _map_type(raw_type, _PY_TYPES, "?") != "?"
                else f"  # type: {raw_type}"
            )
...
def generate_prisma_schema(snapshot: dict[str, Any] | None) -> str:
    ...
    for rel in ix["relations"]:
        ...
        for (child_oid, child_col), e in ix["fk_by_child"].items():
            ...
            parent_field = _column_names(parent_cols).get(
                str(e.get("parent_column_name")),
                _snake_name(e.get("parent_column_name"), "column"),
            )
        ...
        if len(pk_cols) > 1:
            ordered = [
                col_names[str(c.get("column_name"))]
                for c in cols
...
_TS_TYPES: list[tuple[re.Pattern[str], str]] = [
    (
        re.compile(
            r"^(big)?serial|^(small|big)?int|^integer|^(real|float|double|numeric|decimal)"
        ),
        "number",
    ),
    (re.compile(r"^bool"), "boolean"),
...
def generate_typeorm_entities(snapshot: dict[str, Any] | None) -> str:
    ...
    for rel in ix["relations"]:
        ...
        entity_args = (
            json.dumps(table)
            if schema == "public"
            else f"{{ name: {json.dumps(table)}, schema: {json.dumps(schema)} }}"
        )
        ...
        for col in cols:
            ...
            decorator = (
                f"{decorator}({{{', '.join(options)}}})"
                if options
                else f"{decorator}()"
            )
            comment = (
                ""
                if _map_type(raw_type, _TS_TYPES, "?") != "?"
                and ts_type != "string"
                or raw_type.lower().startswith(("text", "varchar", "char"))
                else f" // {raw_type}"
            )
            ...
            lines.append(
                f"  {field_name}{'?' if nullable else '!'}: {ts_type}{' | null' if nullable else ''};{comment}"
            )
            lines.append("")
        for (child_oid, child_col), e in ix["fk_by_child"].items():
            if child_oid != oid:
