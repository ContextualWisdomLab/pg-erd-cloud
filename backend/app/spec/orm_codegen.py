...
def generate_prisma_schema(snapshot: dict[str, Any] | None) -> str:
    ...
    ix = _index(snapshot)
    relations = list(ix["relations"])
    model_requests = [
        {
            "key": f"model:{rel.get('relation_oid')}",
            "kind": "model",
            "namespace": "models",
            "source": str(rel.get("relation_name")),
            "preferred": preferred_prisma_name(_class_name(str(rel.get("relation_name")))),
        }
        for rel in relations
    ]
    model_alloc = allocate_prisma_identifiers(model_requests)
    if not model_alloc["ok"]:
    ...
    relation_alloc = allocate_prisma_identifiers(relation_requests)
    if not relation_alloc["ok"]:
        return PRISMA_EXPORT_FAILURE_SCHEMA

    field_requests: list[dict[str, str]] = []
    for rel in relations:
        oid = rel.get("relation_oid")
        ...
        for col in ix["cols_by_oid"].get(oid, []):
            ...
            field_requests.append(
                {
                    "key": f"field:{oid}:{name}",
                    "kind": "field",
                    "namespace": f"fields:{model}",
                    "source": name,
    ...
    for rows in ix["fk_groups"]:
        ...
        reverse_field = f"{child_model[0].lower() + child_model[1:]}s"
        field_requests.extend(
            [
                ...
                },
                {
                    ...
                    "preferred": reverse_field,
                },
            ]
        )
    field_alloc = allocate_prisma_identifiers(field_requests)
    if not field_alloc["ok"]:
        return PRISMA_EXPORT_FAILURE_SCHEMA
    ...
    lines = ["// Generated by pg-erd-cloud — Prisma schema (edit freely)"]
    if schemas:
        lines += [
            "generator client {",
    ...
    if schemas:
        lines.append(f"  schemas = {json.dumps(schemas)}")
    lines.append("}")
    ...
    for rel in relations:
        ...
        for col in cols:
            name = str(col.get("column_name"))
            field_name = field_alloc["names"].get(f"field:{oid}:{name}", _snake_name(name, "column"))
            raw_type = str(col.get("data_type") or "")
            ...
            if name in pk_cols and len(pk_cols) == 1:
                attrs.append("@id")
            if field_name != name:
                attrs.append(f"@map({quote_prisma_string(name)})")
            comment = (
                ""
                if _map_type(raw_type, _PRISMA_TYPES, "?") != "?"
                and p_type != "String"
                or raw_type.lower().startswith(("text", "varchar", "char"))
                else f" // {raw_type}"
            )
            lines.append(
        ...
        for rows in children_of.get(oid, []):
            ...
            if relation_name is None or field is None:
                continue
            lines.append(
                f"  {field} {cmodel}[] @relation({quote_prisma_string(relation_name)})"
            )
        if len(pk_cols) > 1:
            ordered = [
                field_alloc["names"].get(
                    f"field:{oid}:{c.get('column_name')}",
                    _snake_name(c.get("column_name"), "column"),
                )
                for c in cols
                if str(c.get("column_name")) in pk_cols
            ]
