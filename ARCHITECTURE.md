...
| Live preflight/apply worker | Read-only evidence, locked execution, recovery | Bounded structured read-query, canonical snapshot/base-digest comparison, DB-durable hashed attempt acquire/renew/finish primitives, and deterministic structured existing-table lock-plan compilation are **Implemented**; `execute_bound_live_preflight` binds a caller-owned capture callback and checks to one read-only repeatable-read transaction, and `complete_live_preflight` derives the only valid terminal CAS classification. Consumer-to-attempt binding is **Implemented** as an execution-neutral dual-lease adapter. The lock compiler parses no SQL and acquires no lock; application startup wiring, credential binding, worker execution, target lock acquisition, apply-time revalidation, and apply remain **Planned**. |
