...
| Live preflight/apply worker | Read-only evidence, locked execution, recovery | Bounded structured read-query and canonical snapshot/base-digest comparison primitives **Implemented**; `execute_bound_live_preflight` binds a caller-owned capture callback and checks to one read-only repeatable-read transaction, and `complete_live_preflight` revalidates the immutable plan, requires one exact result for every persisted precondition, and derives the only valid terminal CAS classification. Durable worker identity/attempt binding and apply remain **Planned**. |
...
- a bounded live-preflight primitive compiles only the plan's structured
  ...
  identity, durable attempt acquisition, or DDL authority;
