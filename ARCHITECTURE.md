...
| DBML parser and DDL export | Convert authenticated design text into snapshot JSON and reviewable PostgreSQL/Snowflake DDL | **Implemented export boundary**; PostgreSQL identifiers fail closed on malformed/ambiguous/NUL/over-63-byte input, preserve valid doubled quotes and punctuation, and pass through the dialect-owned identifier renderer. It opens no target connection and grants no apply authority. |
