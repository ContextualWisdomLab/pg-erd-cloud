...
The stored-PostgreSQL preflight provider now performs post-connect
revalidation: after guarded connection acquisition it repeats the exact
encrypted target/snapshot/attempt lookup and requires an identical result
before any target read. It closes the connection on mismatch without granting
capture authority. A concurrent change after that second check remains
