...
| FE-INV-005: isolated DDL and live read-only preflight | `backend/app/forward/isolated_dry_run.py` verifies and executes only a signed all-transactional plan on a caller-owned sandbox connection; `backend/app/forward/live_preflight.py` implements execution-neutral structured reads plus strict snapshot/base-digest comparison | isolated unit rollback/cancellation/fail-closed tests plus PostgreSQL 14–18 exact-plan/catalog convergence in a dedicated ephemeral database distinct from metadata; live-preflight query/limit/transaction/timeout/error tests | [ADR-0002](adr/ADR-0002-isolated-dry-run-and-preflight.md), [UML](UML.md), [Threat model](security/forward-engineering-threat-model.md) | **Partially implemented release blocker.** Deployed sandbox provisioning/materialization/egress/cleanup, worker identity/attempt binding, and live-target no-DDL audit are absent; legacy rollback-on-live is explicitly not evidence. |
| FE-INV-006: fingerprint revalidation for dry run/apply | Isolated execution checks a strict materialized-base digest before DDL and strict target digest after commit; live comparison and terminal CAS persist/revalidate the observed live base | `test_forward_isolated_dry_run.py`; PostgreSQL 14–18 integration; live-preflight match/drift and durable CAS tests | [TRD](TRD.md), [Runbook](runbooks/forward-engineering.md) | **Partially implemented release blocker.** Durable sandbox worker binding, fresh live-target capture/connection/attempt binding, and in-lock apply revalidation remain Planned. |
...
| Real PostgreSQL/version evidence | The 14–18 matrix now separates migrated metadata, DDL sandbox, and read-only preflight target databases while proving exact-plan convergence, run/outbox, and preflight reads; it does not prove deployed lifecycle, production credentials, privileges, concurrency, failures, or representative sizes. | Adversarial catalogs, least-privilege identities, external writers, cleanup/fault injection, representative sizes |
