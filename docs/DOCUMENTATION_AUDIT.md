...
| Real PostgreSQL/version evidence | The 14–18 matrix now proves the bounded exact-plan DDL/catalog convergence path, run/outbox, and preflight reads, but not deployed lifecycle, privileges, concurrency, failures, or representative sizes. | Adversarial catalogs, least-privilege identities, external writers, cleanup/fault injection, representative sizes |
