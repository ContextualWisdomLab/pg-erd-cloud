...
| FE-INV-006: fingerprint revalidation for dry run/apply | Plan stores base digest; pure strict snapshot comparison returns canonical observed digest and match result | `test_forward_live_preflight.py` match/drift and fail-closed snapshot cases | [TRD](TRD.md), [Runbook](runbooks/forward-engineering.md) | **Partially implemented release blocker.** Fresh target capture/attempt binding, durable transition, and in-lock apply revalidation remain Planned. |
