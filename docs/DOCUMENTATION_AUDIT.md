...
| FE-INV-009: durable idempotent run, no apply replay | `models.py`; migration `0010`; `forward/migration_run.py`; `jobs/migration_dispatch_relay.py`; `api/migration_runs.py` | `test_forward_migration_run.py`; `test_migration_dispatch_relay.py`; `test_api_migration_runs.py` | [ADR-0004](adr/ADR-0004-durable-runs-and-recovery.md), [Data model](DATA_MODEL.md), [Runbook](runbooks/forward-engineering.md) | **Partially implemented:** durable identity, atomic identifier-only dispatch outbox, dry-run creation, bounded UUID-only queue publication, CAS/cancellation writers, event chain, and polling exist; scheduled relay operation, queue consumption, execution, and apply no-replay recovery remain blockers. |
...
|---|---|---|
| Scheduled outbox relay/queue execution and apply-run API | Atomic identifier-only outbox persistence, authorized dry-run creation/cancellation, bounded UUID-only queue publication, CAS writers, and integrity-checked polling exist, but continuous relay/consumer/worker execution and public apply creation are unavailable. | Relay-loop lifecycle, consumer restart/cancellation integration tests, approval-bound apply creation |
