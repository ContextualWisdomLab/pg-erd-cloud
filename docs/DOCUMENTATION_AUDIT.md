...
| FE-INV-007: in-lock data preconditions | Compiler emits precondition metadata; `forward/apply_lock_plan.py` emits deterministic existing-table locks; `forward/pre_apply_revalidation.py` binds the signed plan/version/digests to lock-covered checks and fail-closed complete positional observation assessment | Compiler metadata/risk tests, target-free manifest tamper/cross-table/unlocked-check and observation-shape/binding tests, plus test-only PostgreSQL 14–18 compiled-lock/concurrent-insert/check/rollback semantics acceptance | [ADR-0003](adr/ADR-0003-plan-execution-segmentation.md), [Runbook](runbooks/forward-engineering.md) | **Partially implemented input/assessment boundary with ephemeral database-semantics evidence;** production target/lock orchestration, fresh snapshot/privilege capture, connection/lock proof, complete same-connection revalidation, and deployed concurrency proof remain absent. |
