...
| FE-INV-005: isolated DDL and live read-only preflight | `backend/app/forward/live_preflight.py` implements execution-neutral structured reads plus strict snapshot/base-digest comparison | `test_forward_live_preflight.py` proves exact query/limit/transaction/timeout/error and match/drift contracts | [ADR-0002](adr/ADR-0002-isolated-dry-run-and-preflight.md), [UML](UML.md), [Threat model](security/forward-engineering-threat-model.md) | **Partially implemented release blocker.** Sandbox, worker identity/connection, worker-owned fresh capture binding, and real-target no-DDL evidence are absent; legacy rollback-on-live is explicitly not evidence. |
| FE-INV-006: fingerprint revalidation for dry run/apply | Plan stores base digest; pure strict snapshot comparison returns canonical observed digest and match result; terminal dry-run CAS persists it and rejects `passed` mismatch / `drifted` match after plan-integrity revalidation | `test_forward_live_preflight.py` match/drift and fail-closed snapshot cases; `test_forward_migration_run.py` durable match/mismatch/CAS cases | [TRD](TRD.md), [Runbook](runbooks/forward-engineering.md) | **Partially implemented release blocker.** Fresh target capture/connection/attempt binding and in-lock apply revalidation remain Planned. |
...
| Isolated sandbox and read-only preflight | The structured read and snapshot comparison primitives have no credential/worker authority and a rollback on production still creates lock/scan/rewrite risk. | Network/credential isolation proof, real PostgreSQL sandbox convergence, worker-owned fresh capture binding, and live no-DDL audit |
