...
| FE-INV-007: in-lock data preconditions | Compiler emits precondition metadata; `forward/apply_lock_plan.py` emits deterministic existing-table locks; `forward/pre_apply_revalidation.py` binds the signed plan/version/digests to lock-covered checks | Compiler metadata/risk tests and target-free revalidation manifest tamper/cross-table/unlocked-check tests | [ADR-0003](adr/ADR-0003-plan-execution-segmentation.md), [Runbook](runbooks/forward-engineering.md) | **Partially implemented input boundary;** target lock acquisition, same-connection revalidation, and concurrency proof remain absent. |
