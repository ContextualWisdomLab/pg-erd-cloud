...
| FE-INV-005: isolated DDL and live read-only preflight | `backend/app/forward/live_preflight.py` implements only the execution-neutral structured read primitive | `test_forward_live_preflight.py` proves exact query/limit/transaction/timeout/error contracts | [ADR-0002](adr/ADR-0002-isolated-dry-run-and-preflight.md), [UML](UML.md), [Threat model](security/forward-engineering-threat-model.md) | **Partially implemented release blocker.** Sandbox, worker identity/connection, fresh fingerprint binding, and real-target no-DDL evidence are absent; legacy rollback-on-live is explicitly not evidence. |
...
| Isolated sandbox and read-only preflight | The structured read primitive has no credential/worker authority and a rollback on production still creates lock/scan/rewrite risk. | Network/credential isolation proof, real PostgreSQL sandbox convergence, fresh fingerprint binding, and live no-DDL audit |
