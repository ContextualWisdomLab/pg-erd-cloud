...
| FE-INV-006: fingerprint revalidation for dry run/apply | Isolated execution checks a strict materialized-base digest before DDL and strict target digest after commit; `execute_bound_live_preflight` binds caller-owned fresh capture and checks to one read-only repeatable-read transaction; `complete_live_preflight` validates that exact result and server-derives terminal CAS classification | `test_forward_isolated_dry_run.py`; PostgreSQL 14–18 integration; live-preflight same-transaction match/drift, rollback/cancellation, strict result classification, and durable CAS tests | [TRD](TRD.md), [Runbook](runbooks/forward-engineering.md) | **Partially implemented release blocker.** Durable sandbox worker binding, deployed target credential/routing isolation, worker/attempt binding around the caller-owned primitive, and in-lock apply revalidation remain Planned. |
