...
- bounded UUID-only ready-to-processing claim, expiry reclaim, exact lease
  ...
  **Implemented**: exact renewal runs while the injected handler is active;
  ...
  DNS/SSRF guard. Application startup, deployed credential/network isolation,
  unmodified guarded-route integration, and worker execution remain **Planned**;
