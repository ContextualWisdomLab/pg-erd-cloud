...
| FE-TRD-006–008 | Isolated dry-run execution/convergence core, bound read-only live preflight, and transactional compiler subset | `test_forward_isolated_dry_run.py`, `test_forward_live_preflight*.py`, `test_postgres_migration_run_integration.py` (PostgreSQL 14–18) | Sandbox provisioning/materialization/isolation/cleanup, credential-bound worker execution, in-lock precondition repetition, and live apply |
| FE-TRD-009–012 | Durable run/event/outbox/attempt persistence, UUID-only dispatch, exact signal/attempt leases, dry-run/apply-intent/cancellation APIs, expiry, and bounded preflight contracts | `test_forward_migration_run.py`, `test_api_migration_plans.py`, `test_api_migration_runs.py`, `test_migration_dispatch_*`, `test_migration_run_consumer.py`, `test_valkey_queue_integration.py`, `test_postgres_migration_run_integration.py` | Application startup/credentials/deployed worker, crash recovery/no-replay reconciliation, live apply/convergence |
