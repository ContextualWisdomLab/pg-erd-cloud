...
- bounded UUID-only ready-to-processing claim, expiry reclaim, exact lease
  ...
  **Implemented**: exact renewal runs while the injected handler is active;
  ...
  stored-PostgreSQL live-preflight provider is implemented but unwired;
  application startup, deployed credential/network isolation, provider-backed
  supported-version acceptance, and worker execution remain **Planned**;
