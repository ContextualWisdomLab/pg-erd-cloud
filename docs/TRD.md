...
| FE-TRD-007 | Live preflight is read-only evidence; apply repeats fingerprint/data preconditions after locks on the execution connection. | **Partially implemented:** bounded structured boolean reads, strict snapshot comparison, and durable hashed attempt ownership exist; `execute_bound_live_preflight` binds capture/checks to one read-only repeatable-read transaction and completion matches every persisted precondition. The identifier-only live-reader request carries its exact refreshed run state version, and `guard_live_preflight_handoff` can atomically match the exact run/plan/project/target/active-attempt tuple, cancellation, version, lease, digest and expiry in one fresh database statement. PostgreSQL 14–18 acceptance composes the durable handler with a separately constrained test reader; concrete provider composition, the remaining observation-to-target-open gap, deployed credential/route binding, startup/worker operation, and in-lock apply repetition remain Planned. |
