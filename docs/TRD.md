...
- bounded UUID-only ready-to-processing claim, expiry reclaim, exact lease
  ...
  **Implemented**: exact renewal runs while the injected handler is active;
  ...
  impossible. Its guarded connection-acquisition timeout is injected through
  the durable-attempt and UUID-only consumer factories, restricted to finite
  `(0, 60]`-second values, and validated before metadata or target I/O. The
