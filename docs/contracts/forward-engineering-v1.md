...
| FE-AC-003 | Exact stored plan executes successfully in an isolated compatible PostgreSQL sandbox and reaches `target_digest`. | dedicated ephemeral PostgreSQL 14–18 integration database, separate from metadata and target databases; `test_real_postgres_durable_worker_binds_separate_sandbox_and_reader` drives the durable handler through test-owned injected capabilities for the sandbox and separately constrained live reader | Partially implemented core and test-owned worker orchestration evidence; deployed provisioning, dependency materialization, isolation/egress proof, credential resolution, cleanup, startup, and worker operation remain Planned |
