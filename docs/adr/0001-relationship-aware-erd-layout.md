...
The first production slice needs relationship-aware table placement, deterministic coordinates, cycle tolerance, disconnected-component placement, a one-step undo boundary, conservative pre-measurement geometry, and safe failure behavior. Precise column-port routing, obstacle-avoiding orthogonal edges, compound domain groups, and incremental pinned-node layout are separate product capabilities and must not be implied by this decision.
...
5. Measured dimensions are used when available; explicit dimensions are the next source. Before measurement, the height estimate accounts for the table comment, every rendered column row, visible column comment and example lines, column overflow, the index section, rendered index rows, and index overflow. The estimate is intentionally conservative because browser text wrapping is not known before render.
...
## Commercial-license and distribution boundary
...
Vite also copies an exact, byte-identical notice from `frontend/public/THIRD_PARTY_NOTICES.md` into the production bundle as `dist/THIRD_PARTY_NOTICES.md`. The production build fails when the distributed notice is missing or differs from the reviewed source notice.
...
- the source and public distribution notices differ.
...
- Cycles, repeated or empty edge IDs, parallel foreign keys, disconnected tables, missing edge endpoints, rich column metadata, and index rows are covered by executable tests.
- UI tests verify applied coordinates, exact one-step Undo restoration, and failure preservation of both current coordinates and the previous Undo boundary.
- Layout failures cannot partially overwrite saved or manually arranged coordinates.
- Commercial-use permission, repository notice retention, and production-bundle notice distribution are explicit and continuously checked.
...
- Pre-measurement height is conservative rather than pixel-exact; the later explicit layout uses React Flow's measured sizes when available.

## Verification

- `frontend/src/erd/dagreLayout.test.ts`
- `frontend/src/erd/convertLayout.test.ts`
- `frontend/src/App.coverage.test.tsx` applied-coordinate, Undo, and layout-failure scenarios
- `frontend/scripts/verify-layout-license.mjs`
- `frontend/scripts/verify-layout-distribution.mjs`
