...
| Valkey 8 official release and container image | Queue-signal sorted-set semantics, UUID-only ready/processing isolation, exact lease-token stale-ack rejection, retry release, and acknowledgement are verified through the production adapter against a digest-pinned official image. This does not prove production topology, failover, or worker recovery. | Integration-test runtime reference; scheduled publisher, signal lease primitives, and execution-neutral consumer contract Implemented; application consumer wiring/deployment failover/worker evidence Planned |
...
| Live preconditions | Table-empty, no-NULL, and castability preconditions are represented. | Implemented plan metadata; live enforcement Planned |
| Transaction capability | Current emitted statements declare `transactional: true`; blockers set executable `statements=[]`. Supported deltas may remain as review-only `proposed_statements`, with their risks included. | Implemented compiler subset; executor/segment Planned |
