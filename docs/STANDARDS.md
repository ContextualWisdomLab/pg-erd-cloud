...
| Valkey 8 official release and container image | Queue-signal sorted-set semantics, UUID-only ready/processing isolation, monotonic exact lease renewal, stale-token rejection, retry release, and acknowledgement are verified through the production adapter against a digest-pinned official image. The execution-neutral consumer also verifies automatic heartbeat and handler cancellation on lease loss. Separate PostgreSQL-backed hashed attempt ownership and consumer-to-attempt binding are implemented, but this does not prove production topology, startup wiring, failover, credentials, or worker recovery. | Integration-test runtime reference; scheduled publisher, signal lease/consumer contract, DB attempt primitives, and dual-lease binding Implemented; application startup/deployment failover/worker evidence Planned |
...
| Live preconditions | Table-empty, no-NULL, and castability preconditions are represented. | Implemented plan metadata; bounded live-preflight execution and completion CAS are Implemented; durable worker binding and apply-time in-lock repetition remain Planned |
| Transaction capability | Current emitted statements declare `transactional: true`; blockers set executable `statements=[]`. Supported deltas may remain as review-only `proposed_statements`, with their risks included. | Implemented compiler subset; bounded all-transactional isolated executor core is Implemented; deployed sandbox worker and apply segmentation remain Planned |
| Drift control | Plans store base/target digests and bind a succeeded snapshot. | Implemented provenance; pre-apply revalidation Planned |
