...
| Live preconditions | Table-empty, no-NULL, and castability preconditions are represented. | Implemented plan metadata; bounded live-preflight execution and completion CAS are Implemented, as are the signed-plan lock-covered revalidation manifest and fixed parameterized privilege probes that re-derive it from the exact signed plan; production privilege capture, target lock acquisition, durable apply-worker binding, and apply-time in-lock repetition remain Planned |
