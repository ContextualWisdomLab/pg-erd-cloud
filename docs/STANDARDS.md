...
| Valkey 8 official release and container image | Queue-signal sorted-set semantics, UUID-only ready/processing isolation, monotonic exact lease renewal, stale-token rejection, retry release, and acknowledgement are verified through the production adapter against a digest-pinned official image. The execution-neutral consumer also verifies automatic heartbeat and handler cancellation on lease loss. This does not prove production topology, failover, DB-durable worker attempts, or worker recovery. | Integration-test runtime reference; scheduled publisher, signal lease primitives, and execution-neutral consumer contract Implemented; application consumer wiring/deployment failover/worker evidence Planned |
