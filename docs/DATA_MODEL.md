...
| Run/event state tokens, sequence numbers, and digest links are valid. | Database checks constrain predecessor presence and lowercase SHA-256 shapes; the exact application transition graph and CAS writer match UUID, kind, state, state version, and prior event anchor before appending the same-version event; event sequence is unique per run and polling recomputes every canonical digest. | Implemented persistence and polling boundary; workers Planned |
