...
| `PgErdCloudBillingWebhookFailures` | `ticket` | 10분 동안 billing webhook rejected_auth/rejected_config > 0 | 결제/계약 event 반영 실패, 미납/플랜 변경 상태 불일치 가능 | provider/gateway secret, raw-body signature, event replay, support diagnostics 확인 | Billing owner |
| `PgErdCloudLlmDraftFailures` | `ticket` | 10분 동안 LLM draft configuration/provider/prompt-size 실패 > 0 | LLM 보조 산출물 생성 실패 또는 제3자 provider 비용/연동 문제 가능 | `event=llm_draft_usage`, provider key, prompt size, `SHARE_LINK_LLM_DRAFT_ENABLED` 상태 확인 | Product + Backend owner |
