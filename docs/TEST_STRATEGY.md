...
| Roles/legacy apply | `app/permissions.py`, `app/api/connections.py`; `tests/test_permissions.py`, `tests/test_api_apply_sql.py` | `deployer` ordering, default-deny persistent legacy apply before credential access, explicit opt-in compatibility, conservative SQL rejection and DSN-redacted errors. | Implemented default-deny transitional path; it is not target workflow evidence. |
