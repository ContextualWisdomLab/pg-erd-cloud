...
| PostgreSQL integration | Prove real catalog mapping, executable SQL, locks, timeouts, transactions, privileges, fingerprinting, and convergence. | PostgreSQL 14–18 execute an exact signed v1 plan in one transaction and re-introspect strict base/target digests; run/outbox and live-preflight reads share the matrix. Provisioning/cleanup/egress and apply concurrency evidence remain absent. | Ephemeral PostgreSQL 14, 15, 16, 17, and 18 matrix plus separate sandbox lifecycle, privilege, concurrency, cancellation, crash, and cleanup evidence. |
