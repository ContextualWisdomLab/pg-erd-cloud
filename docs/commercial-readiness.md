...
2. **P2 라이선스/과금 운영 연동**: `LICENSE_MODE` 기반 차단은 구현됐으나, 실제 과금·좌석·요금제/요금제 변경·재발급 키 회수 자동화는 미구현.
...
| P1 | 라이선스·사용량·과금 체계 | 문서 초안 단계 | 계정별 라이선스 메타데이터 스키마 초안 + 키 회수/교체/비정상 탐지 SOP |
...
- 🟡 결제/라이선스: `LICENSE_MODE`, `LICENSE_KEY` 기반 기본 키 게이트를 도입했으며
  좌석/과금 연동 정책은 추가 설계 중
