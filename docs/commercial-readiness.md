...
현재 기준일(UTC): 2026-07-02 03:37

### No-Go 항목 (판매 즉시 블로커)

1. **P0 인증·인가 완성도**: 현재는 OIDC 검증 강제, API 허가, 공유 경로 제한이 적용되어 있으나, 사용자 메시지/알림 임계치 템플릿은 미완.
2. **P2 라이선스/과금 운영 연동**: `LICENSE_MODE`와 Ed25519 서명 토큰 검증은 구현됐으나, 실제 과금·좌석 사용량 집계·요금제 변경·재발급 키 회수 자동화는 미구현.
3. **지원/법무 패키지의 계약 텍스트 강제화**: 템플릿 초안은 준비되었으나, 영업/계약에 투입 가능한 문구로 대체되지 않음.
4. **고급 UI 회귀/접근성 자동화 게이트**: E2E/회귀/접근성 smoke 테스트를 PR merge 조건으로 묶지 못함.
...
| P1 | 라이선스·사용량·과금 체계 | 서명 토큰 검증 도입 | 계정별 사용량 집계 스키마 + 키 회수/교체/비정상 탐지 SOP |
| P1 | 운영 자동화 | 부분 완료 | 장애 대응·백업·마이그레이션 절차를 CI 재시작 플로우와 연결 |
| P2 | 법무 문서 고도화 | 템플릿 단계 | 지역/계약 템플릿 교체 및 계약 승인 게이트 반영 |
| P2 | 품질 게이트 | 부분 완료 | UI E2E/접근성 smoke를 `ci` 또는 별도 릴리즈 워크플로에 통합 |
...
- ✅ 온프레미스 라이선스: `LICENSE_PUBLIC_KEY` 기반 Ed25519 서명 토큰 검증을 추가해
  고객/플랜/만료/활성 시작/시트 claim을 오프라인에서 검증할 수 있음
- 🟡 결제/라이선스: 정적 `LICENSE_KEY`는 기존 배포 호환용으로 유지하며, 실제 좌석
  사용량 집계·과금 포털·재발급/회수 자동화는 추가 설계 중
