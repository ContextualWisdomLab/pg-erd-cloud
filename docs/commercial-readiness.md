...
2. **P2 라이선스/과금 운영 연동**: `LICENSE_MODE`, Ed25519 서명 토큰 검증, CLI 기반 발급/재발급, env 기반 토큰/고객 회수 목록, 사용량 summary API, 기본 사용량 한도 enforcement, billing/reactivation URL 노출, plan-change portal handoff, provider-neutral billing event reconciliation, raw-body HMAC signature 검증은 구현됐으나, provider별 checkout/fulfillment와 계약 상태 적용 어댑터는 미구현.
...
| P1 | 라이선스·사용량·과금 체계 | 서명 토큰 발급 CLI + 검증 + env 기반 회수 목록 + 사용량 summary API + 기본 한도 enforcement + billing/reactivation URL + plan-change handoff + provider-neutral billing event 기록 + HMAC signature 검증 도입 | provider별 checkout/fulfillment + 계약 상태 적용 어댑터 |
