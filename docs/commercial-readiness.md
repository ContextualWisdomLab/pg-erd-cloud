...
2. **P2 라이선스/과금 운영 연동**: `LICENSE_MODE`, Ed25519 서명 토큰 검증, CLI 기반 발급/재발급, env 기반 토큰/고객 회수 목록, 사용량 summary API는 구현됐으나, 실제 과금 포털·요금제 변경 자동화는 미구현.
...
| P1 | 라이선스·사용량·과금 체계 | 서명 토큰 발급 CLI + 검증 + env 기반 회수 목록 + 사용량 summary API 도입 | 교체/비정상 탐지 SOP + 과금 포털 연동 |
...
- 🟡 결제/라이선스: 정적 `LICENSE_KEY`는 기존 배포 호환용으로 유지하며, 실제 과금
  포털·요금제 변경 자동화와 사용량 제한 정책은 추가 설계 중
