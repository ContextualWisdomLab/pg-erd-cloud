...
- Arrow Left and Arrow Up select and focus the previous choice; and
...
No database, API, persistence, color value, visual style, group-creation rule, or assignment behavior changes. Rollback would restore mismatched radio semantics in which every native button remained a separate Tab stop and arrow keys did not operate the group; therefore rollback must revert the radio role as well rather than retaining a partially implemented composite widget.
