...
| API → outbox → queue → worker | Run identity | `migration_run_dispatch` is an identifier-only transactional outbox; due-order lock-scoped claim, dedicated-key publication of only `migration_run_uuid`, and exact-attempt publish CAS are implemented. The future consumer reloads and verifies the stored plan. | Outbox persistence/claim/publisher Implemented; scheduled relay/consumer/worker Planned |
...
| TM-10 | Exfiltrate DSN or row data through errors, logs, events, or metrics. | DSN-derived error redaction and fuzz/property tests exist; run evidence recursively rejects SQL/credential field names and PostgreSQL connection-string values; `migration_run_dispatch` has no payload column; the dedicated migration queue receives only the run UUID; the generic durable worker persists fixed failure codes instead of exception text or unknown job-type values. | Queues prohibit secrets, SQL batches, and row values; handler-specific evidence and worker log review tests enforce bounds. | **Partially implemented:** identifier-only outbox and publisher, run storage/polling, and generic worker failure-storage boundaries exist; migration consumer/worker/metrics paths remain Planned. |
