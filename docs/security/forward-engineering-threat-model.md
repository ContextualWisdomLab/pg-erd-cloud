...
| API/worker → credential boundary | Connection UUID | Decrypt DSN only in process memory after authorization; redact failures. | Implemented for current API paths and the unwired guarded live-preflight provider; deployed worker credential identity remains Planned |
