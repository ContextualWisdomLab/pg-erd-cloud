...
| TM-10 | Exfiltrate DSN or row data through errors, logs, events, or metrics. | DSN-derived error redaction and fuzz/property tests exist; run evidence recursively rejects SQL/credential field names and PostgreSQL connection-string values. | Queues prohibit secrets, SQL batches, and row values; worker log review tests enforce bounds. | **Partially implemented:** run storage/polling boundary exists; queue/worker/metrics paths remain Planned. |
