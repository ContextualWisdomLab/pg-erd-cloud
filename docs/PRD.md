...
| FE-PRD-006 | Detect base drift before dry run and again under apply-time locks before DDL. | **Partially implemented:** the isolated dry-run core validates the materialized base digest before DDL; a target-free manifest binds the signed plan/base/target/PostgreSQL version to deterministic lock targets and structured checks and rejects any check not covered by its statement table lock. Target lock acquisition and same-connection apply-time revalidation remain **Planned** | Injected-drift and concurrency tests |
