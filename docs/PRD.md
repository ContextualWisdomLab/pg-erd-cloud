...
| FE-PRD-007 | Require deployer authority, exact plan/dry-run digests, typed target confirmation, and destructive acknowledgement. | **Partially implemented**; the non-dispatched apply-intent route persists those exact bindings, while apply-time revalidation/execution remain Planned | Role/tamper/race tests |
| FE-PRD-008 | Persist idempotent dry-run/apply resources and append-only evidence; never auto-replay an ambiguous apply. | **Partially implemented**; dry-run and non-dispatched apply intents/resources/evidence, DB-durable hashed attempt CAS, and exact consumer-to-attempt binding exist; application startup wiring, worker execution, crash recovery, and apply execution remain absent | Crash, duplicate-submit, state-machine, and exact-owner lease tests |
