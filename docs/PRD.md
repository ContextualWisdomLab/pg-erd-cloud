...
| FE-PRD-005 | Dry run executes exact stored-plan DDL only in an isolated compatible sandbox; production receives bounded reads only. | **Partially implemented:** execution core, dedicated ephemeral PostgreSQL 14–18 database round trip, and a test-owned durable-handler composition over separate sandbox and read-only target connections exist; deployed provisioning, credential resolution, isolation/lifecycle, startup, and worker operation remain Planned | Network/egress-isolation, cleanup, deployment identity, and live no-DDL evidence |
