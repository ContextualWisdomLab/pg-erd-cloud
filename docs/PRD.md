...
The current repository implements only the first control-plane slice: canonical
model revisions, server-compiled immutable plans, fail-closed subset handling,
and a `deployer` role. Isolated dry run, durable apply, post-apply convergence,
and the frontend workflow are **Planned** and remain production release blockers.
